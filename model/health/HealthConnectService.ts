...
const logHealthError = (scope: string, error: unknown): void => {
  console.error(`[HealthConnect] ${scope} 실패:`, error); // l10n-ignore: Health Connect 개발자 로그
};
...
const readAllRecords = async <T extends RecordType>(
  ...
  if (pageToken) {
    logHealthError(
      `${scope} 페이지 상한(${MAX_RECORD_PAGES}) 도달 — 뒷부분이 잘렸다`, // l10n-ignore: Health Connect 개발자 로그
      new Error(`${collected.length}건까지만 읽음`) // l10n-ignore: Health Connect 개발자 로그
    );
  }
...
class HealthConnectService implements HealthService {
  ...
  public getPermissionStatus = async (): Promise<HealthPermissionStatus> => {
    ...
    try {
      ...
      logHealthError('권한 상태 조회', error); // l10n-ignore: Health Connect 개발자 로그
  ...
  public requestPermission = async (): Promise<HealthPermissionStatus> => {
    ...
    try {
      ...
      logHealthError('권한 요청', error); // l10n-ignore: Health Connect 개발자 로그
  ...
  public queryWorkouts = async ({
    ...
    try {
      const records = await readAllRecords(
        ...
        '운동 세션 조회' // l10n-ignore: Health Connect 개발자 로그
      );
      ...
      logHealthError('운동 목록 조회', error); // l10n-ignore: Health Connect 개발자 로그
  ...
  public getRoute = async (
    ...
    try {
      ...
      // 값은 삼키되 로그로 구분한다.
      logHealthError('경로 매핑', error); // l10n-ignore: Health Connect 개발자 로그
  ...
  private prepare = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (status !== SdkAvailabilityStatus.SDK_AVAILABLE) {
        ...
        logHealthError(
          'SDK 사용 불가', // l10n-ignore: Health Connect 개발자 로그
          new Error(`getSdkStatus=${status} (SDK_AVAILABLE 아님)`) // l10n-ignore: Health Connect 개발자 로그
        );
      ...
      logHealthError('SDK 초기화', error); // l10n-ignore: Health Connect 개발자 로그
      this.initialization = null;
  ...
  private readRoute = async (
    ...
    try {
      ...
      logHealthError('경로 조회', error); // l10n-ignore: Health Connect 개발자 로그
  ...
  private readAggregate = async <T extends AggregateResultRecordType>(
    ...
    try {
      ...
      logHealthError(`지표 집계(${recordType})`, error); // l10n-ignore: Health Connect 개발자 로그
