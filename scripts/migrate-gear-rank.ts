...
async function migrateGearRank() {
  try {
    ...
    console.log('장비 랭킹 데이터 마이그레이션 시작...');
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      console.log(`  - ${gearsSnapshot.size}개의 장비 발견`);
        ...
        if (gearData.isCustom === false) {
          ...
          if (existingData) {
            ...
            gearRankMap.set(gearId, {
              count: 1,
              category: category || '기타', // 카테고리가 없으면 기본값
            });
          }
        } else {
          customGearsSkipped++;
    ...
    console.log(`\n총 ${totalGearsProcessed}개의 장비 처리 완료`);
    console.log(`커스텀 장비 ${customGearsSkipped}개 스킵`);
    console.log(`gear-rank에 추가할 장비 ${gearRankMap.size}개`);

    // gear-rank 컬렉션에 데이터 저장
    console.log('\ngear-rank 컬렉션에 데이터 저장 중...');
    ...
    console.log(`\n✅ 마이그레이션 완료!`);
    console.log(
      `총 ${totalSaved}개의 장비가 gear-rank 컬렉션에 저장되었습니다.`
    );
