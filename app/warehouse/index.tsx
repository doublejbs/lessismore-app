 ...
 *
 * `category`는 홈에서 좁혀 온 1차 카테고리고, `unusedOnly`는 홈 `내 기록`(HM-7)의
 * 안 쓴 장비 수를 눌러 들어온 경우다. 들어가서 다시 고르게 하지 않는다.
 */
export default function WarehousePage() {
  const { category, unusedOnly } = useLocalSearchParams<{
    category?: string;
    unusedOnly?: string;
  }>();
  ...
  return (
    <WarehouseWrapper
      ...
      initialUnusedOnly={unusedOnly === '1'}
    />
  );
}
