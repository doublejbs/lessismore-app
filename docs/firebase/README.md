...
5. Rules 시뮬레이터에서 미존재 문서의 본인 ID 기반 좋아요·투표·신고 `get` 허용과 타인 ID 거부를 확인한다.
6. `community-storage.rules`의 내용을 Storage Rules 편집기에 붙여 넣고 공개 읽기, 본인 JPEG 업로드·삭제, 타인 경로 및 목록 조회 차단을 확인한다.
7. CLI를 사용할 때는 프로젝트 설정 파일에서 이 규칙 파일을 명시한 뒤 `firebase deploy --only firestore:rules,storage`를 실행한다. 실행 전 프로젝트와 대상 파일을 확인한다.
8. 인덱스 쿼리에서 Firebase가 생성한 콘솔 링크를 아래에 기록한다.
