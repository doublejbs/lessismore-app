...
interface Props {
  ...
  spots: CampSpot[];
  // 항목 탭 — 지도 탭은 카메라 이동 + 시트 축소, 선택기는 이 박지 선택으로 이어진다(CS-9).
...
const CampSiteFavoritesListView: FC<Props> = ({ spots, onSelect, onClose }) => {
  const renderItem = ({ item }: { item: CampSpot }) => {
    ...
    return (
      ...
      >
        <PretendardText style={styles.rowName} weight='medium' numberOfLines={1}>
          {item.name}
        </PretendardText>
