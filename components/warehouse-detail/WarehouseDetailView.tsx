...
import WarehouseDetailImageView from './WarehouseDetailImageView';
...
// onLayout 측정 전 폴백 (정보 섹션 대략 높이). 실제 높이는 사진 영역(GD-13) 유무로 갈리는데
// — 사진이 있으면 140pt 남짓, 없으면 `사진 추가` 행만큼 — 폴백은 첫 프레임에만 쓰이고
    ...
    return (
      <>
        ...
        <View style={styles.container}>
          ...
          >
            {/* 내 사진 영역(GD-13)은 정보 블록 **위**에 온다(GD-1). 보유 장비만 —
                카탈로그 장비에는 저장할 문서가 없어 업로드 UI를 두지 않는다.
            ...
            <View onLayout={handleInfoLayout}>
              {/* key로 장비가 바뀌면 GearImageUpload를 확실히 재생성한다 — 모델이 진입 시점
                  ...
                  이전 장비의 사진이 그대로 남는다. */}
              {isAdded && (
                <WarehouseDetailImageView key={gear.getId()} gear={gear} />
              )}
