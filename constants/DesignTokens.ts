...
export const AcgFontSize = {
  /**
   * **레퍼런스 실측 기준**(스크린샷 923px ÷ 논리 폭 393pt = 2.35배).
   *
   * 두 줄 이름의 baseline 간격 57px → 줄간 24pt → 활자 ~18pt가 항목 이름이고, 섹션 제목은
   * 그보다 한 단 위인 ~20, 부제·메타는 ~16/15다.
   *
   * **[정정 이력]** 이름을 19로 뒀다가 18로(실측), 섹션 제목은 15 → 19 → 22 → 24로 올렸다가
   * **20으로 되돌렸다**(2026-08-12). 24는 실측보다 두 단 크다 — 구간 제목이 항목 이름에
   * 묻히는 문제는 **크기가 아니라 굵기·간격**으로 푼다: 섹션 제목만 semibold로 두고 항목
   * 이름은 medium으로 낮추면, 20 대 18에서도 층이 갈린다.
   */
  screenTitle: 24,
  sectionTitle: 20,
  sectionSubtitle: 16,
