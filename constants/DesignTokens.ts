...
export const AcgFontSize = {
  /**
   * **[실측 보정 2026-08-11]** 레퍼런스 목록 스크린샷(폰 목업, 화면 폭 580px ↔ 393pt =
   * 1.48배)을 환산하면 항목 이름 ~18 · 메타 ~15 · 섹션 제목 ~20 · 섹션 부제 ~17이다.
   * 이름을 19에서 18로 내리고 섹션 머리를 이름과 **갈라** 둔다 — 레퍼런스는 섹션 제목이
   * 항목 이름보다 분명히 크고, 한 값으로 묶으면 한쪽을 고칠 때 다른 쪽이 끌려온다.
   */
  /**
   * 섹션 제목은 실측(20)보다 두 단 위인 **24**다(2026-08-11 사용자 지적 2회).
   *
   * 레퍼런스의 섹션은 사진 카드 위에 얹혀 크기 없이도 구분되는데, 이 앱의 목록은 글자만이라
   * 구간 제목(`예정`·`지난`)이 항목 이름(18)에 묻혔다. 22로는 4pt 차이라 여전히 같은 층으로
   * 읽혀, 화면 제목(28)과 항목 이름(18) **사이에 분명한 한 단**을 만든다.
   */
  sectionTitle: 24,
  sectionSubtitle: 17,
  rowTitle: 18,
