...
export const Acg = {
  ...
  // ── 리퀴드 글래스 가산 토큰 (2026-08-11). 위 값들은 이미 여러 화면이 쓰고 있어
  //    그대로 두고, 떠 있는 크롬(플로팅 버튼·하단 바·지도 오버레이·토스트·탭바)이
  //    쓸 값만 아래에 더한다.
  //
  // 밝은 유리 채움. iOS는 `BlurView`가 뒤를 굴절시켜 낮은 채움으로도 면이 서지만,
  // 블러가 없는 Android·Web(비용 대비 결과가 약해 폴백)은 채움만으로 세워야 해 더 올린다.
  glassFillSurface: 'rgba(255,255,255,0.78)',
  glassFillSurfaceOpaque: 'rgba(255,255,255,0.92)',
  // 뒤가 지도 타일처럼 고대비 이미지인 자리. 표준 채움으로는 입력값·아이콘이 지형·도로·
  // 라벨과 겹쳐 읽힌다(2026-08-03 실기기 확인) — 블러 강도만으로는 못 덮어 채움을 따로 올린다.
  glassFillDense: 'rgba(255,255,255,0.9)',
  glassFillDenseOpaque: 'rgba(255,255,255,0.97)',
  // 잉크 유리 — 유리이면서도 화면에서 가장 무겁게 읽혀야 하는 면(주 액션, 토스트).
  // 0.7은 최악(순백 배경) 합성에서도 흰 글자 대비 6.3:1로 AA를 넘는다.
  glassInkFill: 'rgba(26,26,26,0.7)',
  // 토스트처럼 배경을 고를 수 없는 자리 — 같은 최악 합성에서 10:1까지 올린다.
  glassInkFillDense: 'rgba(26,26,26,0.82)',
  glassInkFillOpaque: 'rgba(26,26,26,0.9)',
  /**
   * 시스템 유리(iOS 26 `UIGlassEffect`)에 넘기는 잉크 틴트. 채움 View를 덮는 대신 **색만**
   * 넘겨야 시스템이 그 색을 재질에 녹여 굴절·스펙큘러 하이라이트를 유지한다. 위 채움값보다
   * 알파가 낮은 이유가 그것이다 — 재질이 스스로 확산을 더하므로 색까지 진하면 불투명해진다.
   */
  glassInkTint: 'rgba(26,26,26,0.55)',
  // 스펙큘러 엣지 — RN에 inset 그림자가 없어 **위쪽 테두리만** 밝게 줘 근사한다.
  // 사방 한 겹이면 판때기로 보이고, 위가 밝아야 빛을 받은 유리로 읽힌다.
  glassStrokeTop: 'rgba(255,255,255,1)',
  glassInkStroke: 'rgba(255,255,255,0.18)',
  glassInkStrokeTop: 'rgba(255,255,255,0.42)',
...
export const AcgShadow = {
   ...
   * 종이 면(리스트 행)의 바닥 경계.
   *
   * 원래 6%였다. 지면이 따뜻한 회색(#F4F3EF)일 때는 흰 면 자체가 경계를 만들어 그림자가
   * 거들기만 했는데, **지면이 흰색이 되면서(2026-08-11) 면과 지면이 같은 색**이라 이 선
   * 하나가 행 구분을 통째로 맡는다. 6%로는 보이지 않아 헤어라인이 읽히는 값까지 올린다.
   */
  paper: '0 1px 0 rgba(26,26,26,0.14)',
