...
**타입 6단** — 레퍼런스 실측(24/20/18/16/15/14)에서 **한 단 내린 값**이다. 레퍼런스는 영문이고
이 앱은 한글이라 같은 pt에서 더 크고 빽빽하게 읽힌다(실측 근거: 스크린샷 923px ÷ 393pt = 2.35배).

| 단 | 값 | 자리 |
| --- | --- | --- |
| 화면 제목 | 22 semibold | 탭·화면·시트 제목 |
| 섹션 제목 | 18 semibold | 섹션·구간 머리(`AcgSectionHeaderView`), 부제 15 회색 |
| 항목 이름 | 16 medium | 목록 행 제목 |
| 본문·컨트롤 | 14 | 메타 줄, 칩 라벨, 버튼 라벨, 입력값 |
| 메타 | 13 | 라벨·보조 |
| 수치 | 24·28·30 콘덴스드 | 무게·D-day 등 **비교하는 값**(`AcgDisplayText`) |
