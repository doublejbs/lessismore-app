...
- **카탈로그 장비 이미지는 제공하지 않는다** — 크롤 이미지는 미표시, 장비 공유 갤러리는 폐기 상태를 유지한다. 다만 사용자가 직접 올린 본인 장비 사진은 본인 화면에만 허용한다([specs/DataModel.md](specs/DataModel.md) §1, 2026-07-29 개정). 공개 커뮤니티 게시글 사진은 장비 이미지와 별도 UGC 정책·Storage 경로를 따른다([specs/Community.md](specs/Community.md) CM-6, [specs/DataModel.md](specs/DataModel.md) DM-28 `[제안]`).
...
- 이름 16 medium(두 줄까지) / 메타 14 **잉크**(회색이 아니다 — 무게·브랜드는 정보다).
...
**타입 스케일** — 크기는 레퍼런스 실측(24/20/18/16/15/14)에서 **한 단 내린 값**이다. 레퍼런스는
영문이고 이 앱은 한글이라 같은 pt에서 더 크고 빽빽하게 읽힌다(실측 근거: 스크린샷 923px ÷ 393pt
= 2.35배). 단은 크기·줄간·자간 **세트 토큰 `AcgType`** 이다 — 화면은 `fontSize`/`lineHeight`/
`letterSpacing`을 직접 쓰지 않고 `...AcgType.<단>`을 스프레드한다(2026-08-12, HM-8 타입 스케일).

| 단 | 크기/줄간 | 자간 | 굵기 | 자리 |
| --- | --- | --- | --- | --- |
| 화면 제목 | 22/28 | -0.4 | semibold | 탭·화면·시트 제목 |
| 섹션 제목 | 18/24 | -0.2 | semibold | 섹션 머리(`AcgSectionHeaderView`), 부제 15/21 회색 |
| 항목 이름 | 16/22 | 0 | medium | 목록 행 제목 |
| 본문·컨트롤 | 14/20 | 0 | — | 메타 줄(`rowSubtitle`), 칩·버튼 라벨(`control` — 누르는 라벨은 전부), 입력값 |
| 문단 | 14/21 | 0 | — | 여러 줄 설명·안내문(`body`) |
| 메타 | 13/18 | +0.1 | — | 라벨·보조 |
| 수치 | 24/28 · 28/32 · 30/34 | 0 | 콘덴스드 700 | 무게·D-day 등 **비교하는 값**(`AcgDisplayText`, `AcgType.display*`) |

- 섹션 제목과 항목 이름은 18 대 16으로 2pt 차이다 — 층은 **굵기**가 가른다(semibold vs medium).
  굵기는 `PretendardText`의 `weight` prop으로 지정한다(스타일에 못 싣는다).
- **자간은 크기별이다** — 큰 글자는 음수, 본문 0, 메타는 +0.1. 한 값을 전 크기에 쓰지 않는다.
- 콘덴스드(Archivo Narrow)는 **숫자·라틴 전용**이다. 한글이 섞인 값(`비 23°/30°`·`여행 중`)은
  Pretendard로 두고 한 단 작게 잡는다(줄박스를 수치와 맞추는 커스텀 줄간만 예외적으로 허용).
- `AcgDisplayText`는 `tabular-nums`(비교하는 숫자가 자릿수 폭으로 안 흔들리게) +
  `maxFontSizeMultiplier 1.5`가 기본이다.
- **Dynamic Type**: 글자를 담는 컨트롤(버튼·인풋·칩)은 고정 `height` 대신 `minHeight + 세로 패딩`.
