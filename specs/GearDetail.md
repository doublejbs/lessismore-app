...
- 카탈로그 `gear/{id}.coupangUrl`이 있을 때만 `쿠팡에서 최저가 보기` 링크를 노출한다. 값이 없으면 링크와 고지 문구 모두 미노출.
- 위계상 주요 CTA(창고 추가/수정)와 겹치지 않게 **보조 텍스트 링크**(배경 없는 가운데 정렬 텍스트 + `›`)로 표시하고, **화면 최하단에 고정**(스크롤과 무관하게 항상 하단, 상단 hairline 구분선)한다. 미추가 장비는 하단 고정 바에서 `내 창고에 추가하기` 버튼 **위**에 함께 놓는다(주요 CTA는 최하단 유지).
...
- [ ] `coupangUrl`이 있는 카탈로그 장비 → 상세 하단에 `쿠팡에서 최저가 보기` 보조 링크·수수료 고지 노출, 탭 시 외부 링크 열림
