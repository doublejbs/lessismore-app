...
- 섹션 타이틀은 `블로그·유튜브 후기`(앱 내 사용자 `리뷰` 섹션과 구분). **배치는 보유 여부로 분기**한다 — 미보유(쇼핑 맥락)는 외부 후기가 구매 판단에 더 유용하므로 리뷰(댓글) 섹션 **위**, 보유는 리뷰 섹션 아래. 검색어는 두 소스 모두 `"{제조사 표시명} {장비 표시명} 후기"`(표시명 = `getDisplayName()` 계열, 제조사 없으면 생략).
