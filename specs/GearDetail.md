...
- 카탈로그 `gear/{id}.coupangUrl`이 있을 때만 `최저가 구입하기` 버튼을 노출한다. 값이 없으면 버튼과 고지 문구 모두 미노출.
...
- 버튼 바로 아래에 수수료 고지 문구(`이 링크는 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.`)를 상시 표시한다.
...
- [ ] `coupangUrl`이 있는 카탈로그 장비 → `최저가 구입하기` 버튼·수수료 고지 노출, 탭 시 외부 링크 열림
