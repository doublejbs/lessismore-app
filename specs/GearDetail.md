...
- **유튜브 영상**(상위 4건, 가로 카드)과 **네이버 블로그**(상위 5건, 리스트) 구성·동작은 CS-3와 동일 — 후보 확대(`maxResults=25`/`display=30`) 후 **관련성 필터**를 적용해 상위 건수만 남기고, 유튜브에 `relevanceLanguage=ko`·`regionCode=KR`을 함께 보낸다(규칙은 CS-3 `관련성 필터` 항목이 단일 소스). 탭 시 외부 앱/브라우저로 이동, HTML 태그·엔티티 제거, 소스별 실패·0건이면 해당 리스트만 생략(둘 다 0건이면 섹션 생략).
  - 장비(GD-6)의 **필수 토큰은 장비명 토큰만** 쓴다 — `getDisplayName()`과 `getName()`을 각각 공백으로 분리한 2자 이상 토큰에서 사이즈·성별 등 **일반 토큰**(`mens`·`남`·`엑스라지`·`s`/`m`/`l` 등, 목록은 공용 모듈에 둔다)을 뺀 집합. **제조사만 일치하는 항목은 통과시키지 않는다** — 브랜드는 맞고 제품이 다른 결과(`몽벨 버사자켓` 상세에 `몽벨 아울렛 득템후기`)가 사용자에게 가장 무관하게 읽힌다. 장비명 토큰이 전부 일반 토큰이라 비면 제조사 토큰을 필수 토큰으로 쓴다 — 이때 **제조사는 낱말로 쪼개지 않고 문자열 전체를 한 토큰으로** 쓴다(쪼개면 `노스` 같은 조각이 무관한 제목에 걸린다).
  - **일반 토큰 목록에는 카테고리·품목 명사(`텐트`·`배낭`·`필로우`·`매트`·`파우치` 등)도 포함한다.** 카테고리 명사만으로 판정하면 **같은 품목의 다른 제품**이 그대로 통과한다 — 라이브 실측: `엑스패드 렘 필로우` 상세에서 필수 토큰이 `["필로우"]`뿐이라 `엑스패드 딥슬립 필로우`·`랩 스트라토스피어 필로우`(타사 제품)가 통과했다. 모델명이 함께 있는 장비(`스텔라릿지 텐트`)는 모델명 토큰이 판정을 계속 담당하므로 동작이 바뀌지 않는다.
  - **필수 토큰 폴백 순서**: ① 장비명 낱말 토큰 → ② 비면 **장비명 전체를 공백 없이 이어 붙인 토큰** → ③ 그래도 비면 제조사 토큰. ②가 필요한 이유는 `렘`처럼 **1자 고유 모델명이 길이 규칙(2자)에 걸려 버려지기** 때문이다(카탈로그 10건: `렘 필로우`, `돔 N 텐트 매트`, `콕 파우치`, `더 원/투 텐트`). 라이브 실측에서 `["필로우"]` → 블로그 통과 20건(대부분 타사 제품)이 `["렘필로우"]` → **3건 전부 해당 제품**으로 바뀌었다. ③(제조사)보다 ②를 앞세우는 이유는 제조사 폴백이 `브랜드만 일치` 문제를 되살리기 때문이다.
