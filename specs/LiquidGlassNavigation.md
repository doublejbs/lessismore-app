...
- **[정정] 창고는 네이티브 검색을 쓴다**(2026-07-31). 창고가 탭 루트에서 **푸시 화면**으로 내려가며(Home.md HM-0) large title이 없어져, 아래 기각 사유의 전제가 사라졌다.
  ...
  - 바 버튼 배치는 `[검색][+ 장비 추가]`. Android·Web은 커스텀 헤더 행에 **같은 순서·같은 자리**로 둔다.
...
- iOS 창고 탭 루트: 중첩 Stack은 유지하되 헤더 숨김. **커스텀 타이틀 행** — 좌측 `창고`(32pt bold, large title 톤) + **같은 행** 우측 원형 검색 버튼(44pt) — 탭 시 [검색 필드 + `취소`]로 전환. 필드는 시스템 검색 바 모양(필, `surfaceMuted`)으로 스타일링한다.
- Android·Web도 같은 커스텀 타이틀 행을 쓴다(로고 행 제거, 전 플랫폼 통일 — 사용자 결정). 검색 토글 동작은 기존과 동일.
...
> 2026-08-11. iOS 26 리퀴드 글래스를 **재질로만** 이식하면서 정한 규칙. 위치·크기·모서리·타이포·기능은 이 작업의 범위가 아니다 — 바꾼 것은 떠 있는 표면의 채움뿐이다.

### 원칙

**유리는 "콘텐츠 위에 떠 있는 크롬"에만 입힌다.** 유리의 존재 이유는 *아래로 무엇이 흐르고 있는지* 보여 주는 것이다. 뒤에 흐를 것이 없는 면(지면 위에 그냥 놓인 카드, 시트 안쪽)에 유리를 쓰면 재질값만 치르고 정보는 얻지 못하며, 대비만 떨어진다.

기술 구현은 `components/acg/AcgGlassView.tsx` 하나로 모은다. 채움 계열은 `AcgGlassTint`(`clear`/`ink`), 밀도는 `dense` prop이다. 색은 전부 `constants/DesignTokens.ts`의 `Acg.glass*` 토큰을 참조한다.

### 유리인 표면

| 표면 | 코드 | 틴트·밀도 | 근거 |
| --- | --- | --- | --- |
| iOS 탭 바 | `app/(tabs)/_layout.tsx` `NativeTabs` | 시스템 | UIKit이 그린다. **JS에서 손대지 않는다.** |
| Android·Web 탭 바 | `app/(tabs)/_layout.tsx` `JsTabLayout` | clear (블러 없는 폴백) | 같은 자리의 크롬. `tabBarBackground`로 레이어를 넣고 `tabBarStyle`은 투명으로만 바꾼다 — 높이·패딩·아이콘·색·순서는 그대로. |
| 플로팅 액션 pill | `components/FloatingPillButton.tsx` | `primary`=ink / `secondary`=clear | 목록 위에 떠서 아래 콘텐츠를 가린다. 주 액션은 유리이면서도 화면에서 가장 무거워야 해 잉크 틴트를 쓴다. |
| 하단 고정 액션 바 | `components/bag-detail/BagDetailBottomBar.tsx`, `components/warehouse-detail/WarehouseDetailView.tsx` | clear | 스크롤 콘텐츠가 바 아래로 흐른다. 세이프에어리어까지 유리를 늘리고, 스펙큘러는 **위쪽 엣지 하나**만 준다(사방을 두르면 바가 아니라 떠 있는 상자로 보인다). |
| 탐색 탭 검색 필드 | `components/search/SearchBarView.tsx` | clear | 이미 유리였다. 이번엔 블러 강도와 스펙큘러 엣지만 재질 기준으로 다시 잡았다. |
| 지도 검색 필드·즐겨찾기 버튼 | `components/camp-site/CampSiteMap*OverlayView.tsx` | clear + `dense` | 지도 위에 떠 있는 크롬. 아래 "지도 예외" 참고. |
| 토스트 | `components/toast/ToastView.tsx` | ink + `dense` | 임의의 화면 위에 뜬다. 배경을 고를 수 없어 밀도를 올려 흰 글자 대비를 지킨다(순백 배경 최악 합성에서 10:1). |

### 유리가 **아닌** 표면

목록 행, 카드, 시트 내부 면, 모달 콘텐츠, 입력 필드(위 검색 필드 제외), 지면(`Acg.bg`), 칩, 지도 검색 결과 드롭다운, 지도 로드 실패 배너, 지도 현재 위치 버튼.

- **콘텐츠 레이어다.** 이것들은 떠 있는 것이 아니라 화면 그 자체라 뒤에 흐를 것이 없다. HIG도 유리를 내비 레이어 전용으로 둔다(§5 마지막 항목과 같은 취지).
- **성능.** `BlurView`는 비싸다. 가로 스크롤 목록의 항목마다 유리를 얹으면 스크롤이 무너진다 — 칩이 빠진 이유가 이것이다. 화면당 유리 면은 소수로 유지한다.
- **지도 현재 위치 버튼만 예외적으로 단색이다.** 라임은 이 디자인의 유일한 액센트이고 **면으로만** 쓰는 색이라, 유리로 흐리면 액센트가 죽는다.
- **지도 드롭다운·배너는 전환적**이다. 팬·줌 중에는 떠 있지 않아 유리로 얻을 것이 없고, 검색 결과는 읽어야 하는 콘텐츠다.

### 지도 예외 (밀도)

지도 오버레이는 뒤가 단색 지면이 아니라 지형·도로·라벨이다. 표준 유리 채움으로는 입력값과 아이콘이 그 위에 겹쳐 읽힌다(2026-08-03 실기기 확인). **블러 강도만으로는 덮이지 않으므로** 채움 자체를 `Acg.glassFillDense`까지 올린다(`dense` prop). 테두리도 흰 광택 대신 어두운 헤어라인(`Acg.line2`)이라야 지도 위에서 경계가 읽힌다 — 위쪽 엣지만 유리의 스펙큘러로 남는다.

지도는 팬·줌 중 프레임 예산이 가장 빡빡하다. **오버레이 유리 레이어는 2개(검색 필드, 즐겨찾기 버튼)로 제한한다.**

### 플랫폼

- 블러는 `expo-blur`의 `BlurView`이며 **iOS에서만** 켠다. Android·Web은 비용 대비 결과가 약해 블러 없이 반투명 채움만 쓰고, 대신 채움 불투명도를 iOS보다 높여 가독성을 지킨다(`Acg.glass*Opaque`).
- RN에는 `backdrop-filter`도 inset 그림자도 없다. 광택은 `borderTopColor`를 나머지 세 변보다 밝게 주는 정도로 근사한다.

### 수용 기준

- 위 표의 표면만 유리이고, "유리가 아닌 표면" 목록은 기존 단색 면을 유지한다.
- iOS `NativeTabs` 설정은 변경되지 않는다.
- 유리 이식으로 어떤 요소의 위치·크기·모서리 반지름·타이포·아이콘·핸들러도 바뀌지 않는다.
- 유리 면의 색은 전부 `Acg.glass*` 토큰을 참조하며 컴포넌트에 리터럴 hex/rgba가 없다.
- Android·Web에서 `BlurView` 없이도 각 유리 면 위 글자가 읽힌다.

## 7. 수동 검증 체크리스트 (파일럿 기준)
