...
- **결정(실측 후 변경)**: 네이티브 `headerSearchBarOptions`를 실측한 결과 두 한계로 **커스텀 헤더 행을 채택**한다 —
  ① UIKit 표준 바는 바 버튼이 컴팩트 행에, large title이 그 아래 행에 놓여 **타이틀과 검색 버튼을 같은 행에 둘 수 없다**. ② 검색 닫힘 시 필드가 중간 크기로 줄었다 사라지는 RNS 통합 애니메이션 결함.
- iOS 창고 탭 루트: 중첩 Stack은 유지하되 헤더는 숨기고, **커스텀 타이틀 행** — 좌측 `창고`(32pt bold, HIG large title 톤) + **같은 행** 우측 원형 검색 버튼(44pt, `surfaceMuted`) — 을 그린다. 탭 시 행이 [검색 필드 + `취소`]로 전환된다(직접 제어라 닫힘 애니메이션 결함 없음).
