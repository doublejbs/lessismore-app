...
- **결정(실측 2회 후 확정)**: 네이티브 `headerSearchBarOptions`의 **`placement: 'stacked'`**(설정 앱 문법)를 채택한다.
  - `integratedButton`(원형 검색 버튼)은 ① 바 버튼이 컴팩트 행, large title이 아래 행이라 같은 행 배치 불가 ② 닫힘 시 필드가 중간 크기로 줄었다 사라지는 애니메이션 결함이 있어 기각.
  - 커스텀 헤더 행(타이틀+버튼 같은 행)도 검토했으나 검색 필드가 네이티브가 아니게 되어 기각.
  - **stacked**는 large title `창고` 아래 **네이티브 UISearchBar가 상시 표시**(`hideWhenScrolling: false`)되어 버튼·토글 자체가 없다 — 같은 행 문제와 닫힘 애니메이션 결함이 원천적으로 없고, 활성화/복귀 전환은 시스템 표준.
- iOS 창고 탭 루트: 중첩 Stack + 네이티브 헤더(`headerLargeTitle`) + stacked 검색. 필터·리스트는 automatic 인셋 ScrollView.
- Android·Web은 기존 로고·검색 토글 행을 그대로 유지한다.
