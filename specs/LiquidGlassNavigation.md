...
- **결정(실측 3회 후 확정)**: **커스텀 타이틀 행**을 채택한다. 요구 조건 — ① 타이틀 위 여백 최소 ② 검색은 버튼으로 두고 탭 시에만 필드 노출 ③ 타이틀·버튼 같은 행 — 을 동시에 만족하는 네이티브 검색 API가 없다:
  - `integratedButton`: 바 버튼이 컴팩트 행·large title이 아래 행이라 같은 행 불가 + 닫힘 시 필드가 중간 크기로 줄었다 사라지는 애니메이션 결함.
