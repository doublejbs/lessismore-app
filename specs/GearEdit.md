...
- **`검색으로 추가` 분기**: **카탈로그 검색 모달**을 연다(`/search` 모달, `SearchWarehouse` + `SearchWarehouseView` 재사용, [Search.md](Search.md) SR-1~3). 탐색 경험을 재사용해 **검색어가 없으면 둘러보기 피드, 입력하면 검색 결과**를 보여주며(탐색 탭과 동일 카드), 결과 카드의 담기 동작만 컨텍스트에 맞춘다. 창고 → `/search`, 배낭 → `/search?bagId={bagId}`.
