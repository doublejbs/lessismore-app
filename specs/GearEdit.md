...
- **`검색으로 추가` 분기**: 컨텍스트에 따라 갈린다.
  ...
  - **배낭 컨텍스트(`bagId`)**: **검색 모달(`/search?bagId={bagId}`, `SearchWarehouse` + `SearchWarehouseView` 재사용)** 을 연다. 탐색과 동일하게 검색어가 없으면 둘러보기 피드, 입력하면 검색 결과를 보여주되, 결과에서 선택하면 창고 등록 + **그 배낭에 바로 담는다**(배낭을 다시 고르는 모달 없이). 이미 창고 보유 장비도 재등록 없이 그 배낭에 담고, 이미 그 배낭에 담긴 장비는 비파괴 체크 배지로 중복 담기를 막는다. 완료 시 토스트로 알린다.
