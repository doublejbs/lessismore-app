...
- 언어 설정 화면에서 선택을 바꾸면 → **재시작·remount 없이** 현재 화면과 뒤 스택의 화면, 탭 라벨(`창고`/`탐색`/`여행`/`정보`), 이후 뜨는 알럿·토스트, 모델 getter가 돌려주는 라벨이 전부 새 언어로 표시된다.
...
### L10N-13 장비 이름·브랜드의 로캘 표시 `[제안]`

장비 이름·브랜드는 번역 리소스가 아니라 **데이터의 두 표기**(영문 캐논컬 `name`·`company` / 한글
`nameKorean`·`companyKorean`)를 로캘에 따라 골라 보여준다 (2026-08-23 사용자 제안).

**수용 기준**

- `Gear.getDisplayName()` / `getDisplayCompany()`가 앱 언어를 읽어 표기를 고른다:
  - **ko**: `nameKorean || name` (현행 유지)
  - **en·ja**: `name || nameKorean` (영문 우선, 없으면 한글 폴백)
- 근거 실측(2026-08-23, `/gear` 표본 500건): `name` 영문 95%·빈 값 0·한글 5%(레거시), `company` 영문 100%.
  사용자 추가 장비(`users/{uid}/gears`)의 옛 형태(`name`=한글)는 폴백으로 한글이 보인다 — 데이터 한계로 수용.
- **캐논컬 값의 용도는 불변**: 편집 폼 프리필·중복 키·정렬(`orderBy('name')`)은 `getName()` 그대로
  ([DataModel.md](DataModel.md) DM-3). 이 요구사항은 **표시 getter만** 바꾼다.
- 언어를 읽는 경로는 **순환 import를 피한다** — `Gear`(App→GearStore→Gear 사슬의 말단)가 `app` 싱글톤을
  import하면 순환이므로, `model/l10n/L10nRegistry.ts`(신규)에 L10n 인스턴스를 등록하고 `getAppLanguage()`로
  읽는다. 이 함수는 L10n의 **observable `language`를 읽으므로** observer 컴포넌트가 언어 전환에 반응한다(L10N-2).
- 언어 전환 시 목록·상세·검색 결과의 장비 이름·브랜드가 즉시 바뀐다.
- 박지 이름·설명 등 **콘텐츠는 대상이 아니다**(§1 결정 2 — 한국어 유지).
- [ ] en에서 창고·배낭 상세·검색 결과의 장비 이름·브랜드가 영문으로 보인다(카탈로그 장비)
- [ ] 영문 표기가 없는 레거시 장비는 en에서도 한글로 보인다(빈 칸·키 노출 없음)
- [ ] ko로 되돌리면 즉시 한글 표기로 복귀
- [ ] 이름순 정렬·편집 프리필·중복 판정이 언어와 무관하게 동일하게 동작

### L10N-14 카테고리 표시명 로컬라이즈 `[제안]`

카테고리(침낭·텐트 등)는 Firestore 캐논컬 값이지만, **표시명은 언어를 따른다**
(2026-08-23 사용자 결정 — 3단계에서 보류했던 것을 뒤집음: 필터 칩이 en/ja에서도 한글로 남는 것을 수용하지 않는다).

**수용 기준**

- **값과 표시를 분리한다.** 캐논컬 값(Firestore 필드·비교·저장)은 절대 불변이고, 화면에 닿는 지점만
  표시명 매핑을 거친다. 매핑의 단일 소스는 번역 키 `category.*`(ko는 캐논컬과 동일 문자열, en/ja 번역).
- 표시 진입점을 하나로 모은다 — `getGearFilterName`(model/gear/GearFilterName.ts)이 언어를 읽어 키를
  돌려주도록 바꾸고, 캐논컬이 필요한 소비자는 별도 함수(예: `getGearFilterCanonicalName`)를 쓴다.
  **함수 결과가 Firestore 쓰기·값 비교에 흘러가는 곳이 없는지 전수 확인**이 이 작업의 핵심이다.
...
- [ ] en/ja에서 창고·배낭 상세·배낭 편집·탐색·인기순위·직접 입력의 카테고리 칩·탭·피커가 번역 표시된다
- [ ] 카테고리 필터링·저장·정렬이 언어와 무관하게 동일하게 동작한다(캐논컬 불변)
- [ ] ko 표시가 기존과 동일하다
