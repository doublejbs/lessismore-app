...
- 편집 화면은 창고 전체 장비를 필터/정렬/검색과 함께 보여준다. **검색 인풋**(이름/브랜드 클라이언트 필터, 결과 없으면 `검색 결과가 없어요`)을 `내 장비` 타이틀 아래·필터 칩 위에 둔다([Warehouse.md](Warehouse.md) WH-8과 동일). 정렬은 하단 바텀시트(WH-3).
