...
- 상세의 장비 목록에서 useless 장비는 **행 우측의 44pt useless 로고 마크(50% 투명)** + **행 본문 50% 투명**으로 표시한다. 장비 이미지 미제공([DataModel.md](DataModel.md) §1)으로 썸네일이 폐지돼 오버레이를 얹을 숙주가 없어졌기 때문에, 로고를 행 우측에 독립 배치한다. 행 `accessibilityLabel`에는 `사용 안 함`을 포함해 시각 표기에만 의존하지 않는다.
...
- [ ] useless 장비 행: 우측 44pt 로고 마크(50% 투명) + 본문 50% 투명, `accessibilityLabel`에 `사용 안 함` 포함
