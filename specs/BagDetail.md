...
- 이 화면에는 **정렬 UI가 없고**, 배낭 편집(BD-4)과 `selectedOrderType_bag` 키를 공유해 거기서 고른 정렬을 그대로 물려받는다. `가벼운순`의 0g 뒤로 보내기([Warehouse.md](Warehouse.md) WH-3)가 이 화면에 적용되는 근거도 이 키 공유이므로, 키를 분리하면 함께 검토해야 한다.
