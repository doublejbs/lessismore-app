...
날씨 화면 상단에서 위치를 설정/변경한다. 위치 설정 방법은 **지명 검색 / 현재 위치 / 지도 선택 / 캠프 박지에서 선택** 네 가지다. 어느 방법이든 최종적으로 `BagLocation`(DM-15)을 만들어 저장하며, 이 위치가 곧 날씨 조회 지점이다.

**수용 기준**

- 위치가 **없으면** 검색 입력창 + `현재 위치 사용` + `지도에서 선택` + `캠프 박지에서 선택`을 노출한다.
- 위치가 **있으면** 위치명 칩 + `변경`으로 접어서 표시한다(상단 공간 절약). `변경`을 누르면 빈 검색창(자동 포커스) + 네 액션 + `취소`를 편다.
- **박지 링크 노출**: `spotId`가 있으면 위치명 칩에 `📍`를 붙이고, 칩 탭 시 박지 상세(`/camp-site/{spotId}`)로 이동한다(배낭↔박지 상호 이동). `spotId`가 없으면 칩은 위치명만 표시하고 이동하지 않는다.
...
- **캠프 박지에서 선택**([CampSite.md](CampSite.md) CS-5): 박지 검색 리스트(박지명·지역 표시, `CampSpotStore` 읽기 재사용)에서 박지를 고르면 그 박지 좌표·박지명으로 위치를 저장하고 **`spotId`를 함께 기록**한다(박지 링크, DM-15). 이후 이 배낭 날씨는 박지 위치 기준이 된다.
- **`spotId` 링크 규칙**: 박지에서 설정하면 `spotId`가 붙고, 지명 검색·현재 위치·지도 선택 등 **자유 위치로 바꾸면 `spotId`는 제거**된다(좌표가 박지와 달라지므로 링크가 무의미해짐 — 위치=박지=날씨 일관성 유지). 다른 박지를 고르면 `spotId`가 교체된다.
