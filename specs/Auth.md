...
- 푸터: `탈퇴하기`(로그인 시에만), 앱 버전(`Constants.expoConfig.version` — **cosmetic 값**, [AppLifecycle.md](AppLifecycle.md) 참고), **사업자 정보**(아래), 카피라이트.

- **사업자 정보는 접힘이 기본인 한 줄**로 두고, 탭하면 펼쳐진다.
  - 항상 펼쳐 두면 정보 탭 하단이 법무 텍스트로 무거워진다. 표시 의무는 "접근 가능하게 제공"으로 충족된다.
  - 표시 항목은 다음 넷뿐이다.
  ...
  - **통신판매업 신고번호도 표시하지 않는다** — 현재 신고 대상이 아니라고 보고 있다(앱에서 직접 판매하지 않고 제휴 링크만 노출).
    신고하면 한 줄 추가한다.
