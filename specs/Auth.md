# 인증 / 계정 (로그인 · 약관 · 정보 탭 · 탈퇴)

| 항목 | 내용 |
| --- | --- |
| 상태 | as-built (2026-06-10 코드 기준) |
| ID 프리픽스 | `AU` |
| 주요 코드 | `model/firebase/Firebase.ts`, `components/login/`, `model/login/`, `app/terms-agreement/`, `app/info/index.tsx`, `app/info/delete/` |
| 관련 스펙 | [DataModel.md](DataModel.md), [AppLifecycle.md](AppLifecycle.md), [Community.md](Community.md) |

## 1. 개요

Firebase Auth 기반 인증(Google/Apple/Email), 약관 동의 강제, 정보 탭(계정 관리), 회원 탈퇴를 다룬다.
웹뷰 브릿지는 2026-07-28 폐기됐다(AU-6).

## 2. 화면 및 진입

- 로그인 UI는 별도 라우트가 아니라 **전역 모달**(`LogInView`)이다 — `LogInAlertManager.show()`로 어디서든 호출.
- `/terms-agreement` — 필수 약관 미동의 로그인 사용자가 강제 리다이렉트되는 화면.
- 정보 탭 `app/info/index.tsx` → 탈퇴는 `/info/delete`, 정책은 `/info/policy`, 사업자 정보는 `/info/business`, **데이터 출처는 `/info/data-sources`** `[기획]`.

## 3. 요구사항

### AU-1 로그인 제공자

**수용 기준**

| 제공자 | iOS | Android | Web |
| --- | --- | --- | --- |
| Google | `GoogleSignin.signIn()` (네이티브 SDK) | 동일 | `signInWithPopup` (동적 import) |
| Apple | `expo-apple-authentication` | **미지원** — 로그인 모달에 버튼 자체가 렌더되지 않음 (`Firebase.logInWithApple`의 방어 에러 메시지는 UI로 도달 불가) | `signInWithPopup` |
| Email | `signInWithEmailAndPassword` | 동일 | 동일 |

- 로그인 모달: 기본 모드(Google/Apple 버튼 + 이메일 로그인 링크) ↔ 이메일 모드(이메일/비밀번호 입력). 오버레이 탭으로 닫으면 입력 상태 초기화.
- 이메일 로그인 실패: `이메일 또는 비밀번호가 올바르지 않습니다.`
- 로그인 취소(팝업/시트 닫기)는 조용히 무시한다(에러 UI 없음).
- Auth persistence: 네이티브는 `getReactNativePersistence(AsyncStorage)`, 웹은 `getAuth` 기본값.

### AU-2 신규 사용자 문서 생성

**수용 기준**

- 최초 로그인 시 `users/{uid}` 문서를 생성한다: `termsAgreed/privacyAgreed/marketingAgreed = false`, `createdAt`, `nickname = hiker{0~9999 난수}` ([DataModel.md](DataModel.md) DM-2).

### AU-3 약관 동의

**수용 기준**

- 항목: 필수 4(이용약관, 개인정보 처리방침, 개인정보 수집·이용, 만 14세 이상) + 선택 2(SMS 수신, 마케팅 활용).
- 동의 완료 판정은 필수 4종 모두 true (DM-2).
- 로그인 상태 + 미동의면 앱 어디서든 `/terms-agreement`로 `replace` 리다이렉트된다(`app/_layout.tsx` 가드).
- 동의 저장 시 6개 필드 + `agreedAt`을 기록하고 `/(tabs)`로 이동한다. 저장 실패 시 알럿 후 화면 유지.
- 제출(시작하기) 버튼은 **필수 4종이 모두 체크된 경우에만 활성화**된다(동의 판정 조건과 동일). 선택 2종은 버튼 활성에 영향을 주지 않는다.

### AU-4 정보 탭

**수용 기준**

- **메뉴 목록은 [Home.md](Home.md) HM-8 목록 문법을 따른다**(2026-08-13 개정, as-built).
  행에 면·그림자·자체 좌우 패딩을 두지 않는다 — 글자가 화면 패딩(`AcgLayout.screenPadding` 24)
  정렬선에 붙어 제목·프로필 행과 같은 축에 온다. 행 사이는 `Acg.hairline` 1px(첫 행 제외).
  - 이전(ACG 세대)에는 행이 `흰 종이 면 + 1px 그림자 + 행 간격 12 + 안쪽 패딩 14`였다.
    순백 지면에서 흰 면은 보이지 않아 그림자만 헤어라인처럼 남았고, 면 안 여백이던
    좌우 14가 정렬선만 38pt로 밀었다.
- **화면 타이틀은 로그인 여부와 무관하게 `내 정보`로 고정한다.** 닉네임은 그 아래 프로필 행에 둔다.
  - 예전에는 로그인하면 닉네임이 타이틀 자리를 차지했다. 닉네임은 **`내 정보`의 값**이지 화면 이름이 아니고, 같은 화면인데 로그인 여부로 제목이 바뀌면 위계가 흔들린다.
- 닉네임 설정/수정 모달(`updateNickname`, trim 저장). 편집 아이콘은 **44×44pt 터치 타깃**과 `accessibilityLabel`을 갖는다.
- `서비스 문의` → 카카오 채널 `http://pf.kakao.com/_VJwSn` 외부 브라우저.
- **`데이터 출처` 행을 둔다** `[기획]` (2026-08-17) → 별도 화면 `/info/data-sources`. 공공데이터(공공누리) 출처 표시 의무를 앱 한 곳에서 일괄로 낸다.
  - 자리는 `개인정보 처리방침`·`이용약관` **뒤**, `로그아웃` **앞**이다 — 정책 계열 정보 행 묶음의 마지막이다. 목록 문법·헤어라인·chevron은 다른 행과 같다.
  - 화면 내용: 소스별로 **기관명 · API/데이터셋명 · 라이선스(공공누리 유형) · 원문 링크** 한 줄씩. 현재 항목은 **한국관광공사 TourAPI(KorService2)**(추천 카드 사진 — 공공누리 **제1유형**만 사용)·**한국관광공사 고캠핑**(야영장 정보)·**국립공원공단**(대피소)이다. 근거와 표기 규칙은 [CampSite.md](CampSite.md) CS-10.
  - 소스가 늘면 이 화면에 줄을 더한다. 다만 **이 화면이 개별 사진 옆 표기를 대체하지는 않는다** — 사진이 보이는 화면(박지 상세의 캡션, 홈 추천 섹션 하단 한 줄)에도 표기가 함께 있어야 한다(CS-10).
- `개인정보 처리방침`·`이용약관`은 **별도 화면으로 밀어낸다**(`/info/policy`).
  - 예전에는 처리방침 전문을 이 화면에서 인라인 아코디언으로 펼쳤다. 8개 조항이라 스크롤이 수천 pt로 늘어나고, 접으려면 다시 맨 위로 올라와야 했다.
  - **이용약관도 함께 제공한다.** 가입 시 동의를 받으면서(AU-3) 나중에 볼 경로가 없었다.
- **`로그아웃`은 탭 시 확인 알럿(`로그아웃할까요?` / 확인 라벨 `로그아웃`)을 거친다**(2026-08-13 사용자 결정). 되돌리려면 다시 로그인해야 하는 액션인데 목록 행이라 실수 탭이 쉽다. 공용 `AlertManager`를 쓴다.
- **`로그아웃`은 일반 메뉴와 시각적으로 구분하고 목록 맨 아래에 둔다.**
  - 예전에는 목록 **첫 항목**이었고 `알림 설정`·`서비스 문의`와 같은 스타일이었다. 사용 빈도가 가장 낮고 되돌리려면 다시 로그인해야 하는 액션이 시선이 가장 먼저 닿는 자리를 차지했다.
  - 미로그인 상태의 `로그인`은 반대로 **주 액션**이므로 이 규칙을 적용하지 않는다.
- 푸터: `탈퇴하기`(로그인 시에만), 앱 버전 상세 `버전 {네이티브 버전} ({빌드 번호}) · {OTA 채널}`(예 `버전 2.0.0 (100) · production`; `expo-application`의 `nativeApplicationVersion`/`nativeBuildVersion` + Hot Updater `getChannel()`. OTA 번들이 적용된 상태면 아래 줄에 `업데이트 {번들 ID 앞 8자}`를 추가 — 2026-09-05, 실기기 문의 때 바이너리·채널·번들을 바로 식별하기 위함. 웹은 채널·번들 없이 `버전 x.y.z`), **버전·사업자 정보 한 줄**(아래), 카피라이트.

- **푸터 한 줄에 좌 `버전`, 우 `사업자 정보 ›`를 둔다.** 사업자 정보 전문은 **별도 화면**(`/info/business`)이 맡는다.
  - 예전에는 푸터에서 접었다 펴는 방식이었다. 펼치면 **가운데 정렬된 푸터 안에 좌측 정렬 2열 블록이 끼어 축이 어긋났다.**
  - 표시 항목은 다음 넷뿐이다.

    | 항목 | 값 |
    | --- | --- |
    | 상호 | 마그마 |
    | 대표자 | 장하림 |
    | 사업자등록번호 | 167-58-00828 |
    | 이메일 | doublejbs@naver.com |

  - **사업장 소재지는 표시하지 않는다**(2026-07-30 결정). 사업자등록증 주소가 자택이라 공개하지 않기로 했다.
    단, **통신판매업 신고를 하게 되면 영업소 소재지가 전자상거래법상 표시 의무 항목이 되므로 그때 다시 넣어야 한다.**
  - **통신판매업 신고번호도 표시하지 않는다** — 현재 신고 대상이 아니라고 보고 있다(앱에서 직접 판매하지 않고 제휴 링크만 노출). 신고하면 한 줄 추가한다.
  - **생년월일은 넣지 않는다.** 사업자등록증에 있을 뿐 표시 의무 항목이 아니며, 노출하면 개인정보만 늘어난다.
  - 개인정보 보호책임자 연락처는 개인정보 처리방침 제10조에 이미 있으므로 여기에 중복해서 싣지 않는다.

- **푸터 지형 일러스트** `[제안]` (2026-08-14 사용자 결정)
  - **화면 하단 고정 배경**(홈 지형 문법과 동일, 2026-08-14 사용자 결정)으로
    `assets/images/info-footer-forest.png`(침엽수림 + 너덜 지형, 하프톤, 브랜드 라임 —
    1204×583, 원본 1804×872에서 리샘플)를 스크롤 콘텐츠 뒤에 배치한다.
  - **화면 폭 전체**로, **원본 비율(1804:872) 유지** — 크롭·왜곡 없이 그린다.
  - 장식 이미지다 — `accessible={false}`로 VoiceOver에서 읽히지 않게 한다.
  - 화면 폭 전체·원본 비율을 유지한 채 `position: absolute`와 `bottom`으로 고정한다.
    탭바 위에서 자연스럽게 걸치고, 스크롤 콘텐츠 하단에는 일러스트 높이만큼 패딩을 확보해
    스크롤 끝에서도 마지막 콘텐츠와 겹치지 않게 한다. 콘텐츠가 짧아도 첫 화면부터
    일러스트가 화면 하단에 보여야 한다.
  - **디자인 시스템과의 관계**: `지형 그래픽은 홈에만 둔다`(2026-08-11, [Home.md](Home.md) HM-8
    "지형 그래픽·그레인·텍스처를 쓰지 않는다") 결정의 **예외**다 — 정보 탭 푸터는 목록·카드가
    없는 화면 끝 장식이라 무늬가 콘텐츠와 섞이는 문제가 없어 사용자 결정(2026-08-14)으로
    예외를 추가한다. 홈의 지형(`home-mountain` 등, HM-9)과 같은 계열의 브랜드 라임 일러스트다.

### AU-5 회원 탈퇴

**수용 기준**

- `/info/delete`에서 삭제 데이터 안내(배낭/장비/개인 설정, 복구 불가) 후 확인 → 시스템 알럿 재확인.
- 본인 확인을 위해 **제공자별 재인증**을 수행한다: Google/Apple은 재로그인.
- ⚠ **Email 사용자는 현재 탈퇴 불가**: 탈퇴 화면은 모든 제공자에 `deleteUserAccount()`만 호출하는데 email 제공자는 여기서 예외가 발생한다. 비밀번호 재입력 UI가 없고 `deleteUserAccountWithEmail()`은 호출처 없는 데드 코드다(미해결 질문 참조).
- 재인증 성공 시: Firestore 사용자 데이터 삭제(AU-8 참조) → `deleteUser`(Auth 계정 삭제) → 로컬 상태 초기화 → (네이티브) `GoogleSignin.signOut()` 시도(실패 무시).
- 재인증 취소(`auth/popup-closed-by-user`, code `12501`): `재인증이 취소되었습니다. …` 알럿, 탈퇴 중단.
- 기타 실패: `회원 탈퇴 중 오류가 발생했습니다. 다시 시도해주세요.`
- 완료: `회원 탈퇴 완료` 알럿 후 `/`로 이동.

### AU-6 웹뷰 브릿지 `[폐기]`

**2026-07-28 폐기.** 검색이 네이티브로 전환되면서 웹뷰 진입점이 사라졌고, 라우트 등록만 남은 죽은 코드였다(`/not-login-search`로 이동하는 코드가 앱에 하나도 없었다). `app/not-login-search/`·`components/webview/`·`model/webview/`를 모두 제거했다. 웹(`useless.my`)과의 브릿지 계약(`window.NativeBridge`, `AUTH_TOKENS` 주입 등)도 함께 폐기된다 — 다시 필요해지면 이 절을 새로 쓸 것.

### AU-7 로그인 상태 전파

**수용 기준**

- `onAuthStateChanged` 리스너 + MobX observable로 로그인 상태를 전파하고, 창고/배낭/검색은 reaction으로 자동 갱신한다.

### AU-8 탈퇴 시 데이터 삭제 범위

회원 탈퇴 시 사용자의 실데이터를 안내 문구("모든 데이터가 삭제됩니다")에 맞게 삭제한다.

**수용 기준**

- `deleteUserData(uid)`는 재인증 성공 후 다음을 삭제한다 (`writeBatch`는 500 한도 미만으로 청크 분할):
  1. 사용자 장비 중 카탈로그 장비(`isCustom === false`)의 `gear-rank` count 감소, count ≤ 1이면 문서 삭제 — DM-6 불변 규칙 유지
  2. `users/{uid}.bags`에 기록된 모든 `bag/{bagId}` 문서
  3. `users/{uid}/gears` 서브컬렉션 전체 문서 (Firestore는 부모 문서 삭제 시 서브컬렉션을 자동 삭제하지 않으므로 명시 삭제)
  4. `comment-likes`에서 `userId == uid`인 문서 전체 (`where` 쿼리, 전체 스캔 금지)
  5. `users/{uid}` 문서
- Firestore 삭제가 실패해도 Auth 계정 삭제(`deleteUser`)는 계속 진행한다(현 정책 유지, 콘솔 로그).
- 작성한 댓글(`gear-comments`)은 삭제하지 않는다 — 탈퇴 후에도 닉네임 명의로 남는다 (컬렉션 그룹 인덱스 부재로 클라이언트에서 일괄 조회 불가, 미해결 질문 참조).
- 좋아요 문서 삭제 시 해당 댓글의 `likeCount`는 보정하지 않는다(비정규화 카운트 드리프트 허용, 미해결 질문 참조).

### AU-9 내 정보 진입점 이동 `[제안]`

커뮤니티가 다섯 번째 하단 탭을 사용하므로 기존 `내 정보`는 기능을 유지한 채 홈에서 여는 푸시 화면으로 이동한다([AppLifecycle.md](AppLifecycle.md) APP-9).

**수용 기준**

- `app/info/index.tsx`의 화면 내용을 공개 경로 `/info`로 제공한다.
- 홈 우측 상단에 프로필 아이콘 버튼을 둔다([Home.md](Home.md) HM-15). 탭하면 `/info`로 이동한다.
- 로그인 여부와 무관하게 같은 아이콘·진입점을 사용한다. 미로그인 상태의 `/info`에는 기존처럼 로그인 주 액션을 표시한다.
- 닉네임, 알림 설정, 언어 설정, 서비스 문의, 정책, 사업자 정보, 로그아웃·탈퇴 기능과 기존 화면 문법은 AU-4 그대로 유지한다.
- 로그인 상태에서는 계정 영역에 `내가 쓴 글` 행을 추가로 표시한다(알림 설정 행 위, 셰브론 행 문법). 탭하면 `/community/mine`으로 이동한다([Community.md](Community.md) CM-13, 2026-09-04). 비로그인 상태에서는 행을 숨긴다.
- `/info`에서 뒤로가면 진입한 홈의 스크롤 위치를 유지한다.
- 회원 탈퇴 데이터 범위에 커뮤니티 게시글·사진·댓글·좋아요·투표를 추가한다(CM-12, DM-28). 공개 사진 정리는 재시도 가능한 서버 작업을 사용한다.

## 4. 데이터

- [DataModel.md](DataModel.md) DM-2 (`users/{uid}`).
- Firebase 프로젝트: `lessismore-7e070` (Auth/Firestore/Storage).

## 5. 플랫폼 분기

AU-1 표 참조. 추가로: 토큰 갱신은 네이티브 `GoogleSignin.getTokens()` / 웹 `user.getIdToken(true)`.

## 6. 엣지 케이스

- 로그인 모달 열림 중 취소 → 입력 초기화 후 닫힘.
- 탈퇴 중 Firestore 삭제 실패 → 콘솔 로그만 남기고 Auth 계정 삭제는 계속 진행.

## 7. 수동 검증 체크리스트

- [ ] Google/Email 로그인·로그아웃, iOS·웹에서 Apple 로그인 (Android에는 Apple 버튼 미노출 확인)
- [ ] 신규 계정 → 약관 화면 강제 진입 → 필수 4종 모두 체크 시에만 버튼 활성·메인 진입
- [ ] 필수 중 일부만 체크 → 시작하기 버튼 비활성
- [ ] 약관 동의 후 재실행 → 리다이렉트 없음
- [ ] 닉네임 변경 → 댓글 작성자명에 반영
- [ ] 탈퇴(Google/Apple) → 재인증 취소 시 계정 유지, 완료 시 배낭·장비·좋아요 문서가 모두 삭제되고 재로그인하면 신규 사용자로 생성
- [ ] `[기획]` 정보 탭 `데이터 출처` 행이 정책 행 뒤·`로그아웃` 앞에 있고, 탭하면 `/info/data-sources`가 열린다
- [ ] `[기획]` 데이터 출처 화면에 한국관광공사 TourAPI(KorService2)·고캠핑·국립공원공단 항목과 **공공누리 유형** 표기·원문 링크가 있다([CampSite.md](CampSite.md) CS-10)
- [ ] `[제안]` 정보 탭 푸터 일러스트 — 화면 하단 고정 배경(홈 지형 문법과 동일)으로 첫 화면부터 화면 폭 전체·원본 비율(크롭·왜곡 없음)로 표시, 스크롤 끝의 마지막 콘텐츠와 겹치지 않음, VoiceOver에서 읽히지 않음
- [ ] `[제안]` 홈 프로필 아이콘 → `/info`, 기존 계정·설정·정책 기능 유지, 뒤로가기 시 홈 상태 유지
- [ ] `[제안]` 탈퇴 후 커뮤니티 공개 글·사진·댓글·반응이 정리되고 다른 사용자의 카운트가 보정됨

## 8. 미해결 질문

- 탈퇴 시 **댓글 잔존 + likeCount 드리프트**(AU-8) — 완전 삭제가 필요하면 Cloud Function 또는 `comments` 컬렉션 그룹 인덱스 도입 검토.
- **Email 사용자 탈퇴 불가**(AU-5) — 비밀번호 재인증 UI 추가 필요.
- 이메일 **가입** 플로우 미구현 확정: `createUserWithEmailAndPassword` 래퍼는 호출처가 없다. 기존 이메일 계정만 로그인 가능 — 가입 경로 정책 필요.
- **커뮤니티 탈퇴 정리 서버 작업** `[제안]` — 현재 AU-8은 클라이언트 배치 중심이고 커뮤니티의 공개 사진·교차 게시글 댓글을 완전 정리할 서버 경로가 없다. DM-28 구현 방식과 함께 결정해야 한다.
