...
- 현재 탭 5개: `홈`(house) / `탐색`(magnifyingglass) / `지도`(map) / `여행`(figure.hiking) / `커뮤니티`(person.2.fill). 활성 색상 검정; `내 정보`는 홈 프로필 버튼에서 `/info`로 연다.
...
- [ ] `[제안]` 탭이 `홈 / 탐색 / 지도 / 여행 / 커뮤니티` 다섯 개이고 웹에서는 지도만 숨김
