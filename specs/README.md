...
| [Community.md](Community.md) | `CM` | 커뮤니티 탭 — 질문 / 배낭 후기 / 투표 / 공개 사진 / 댓글·신고 `[제안]` |
...
- **시각 일관성** — 여백·아이콘 크기·모서리 반경·색상·타이포(`PretendardText`)가 같은 화면의 기존 요소 및 유사 화면과 어긋나지 않는가.
  - **레퍼런스 화면 문법(2026-08-11 이식 진행 중)**: 탐색 탭은 레퍼런스 앱에서 뽑은 화면 문법 7개를 따른다 — 정본은 [Feed.md](Feed.md) §9. 다른 탭으로 넓히는 작업은 그 목록을 기준으로 판단하고, 아직 이식하지 않은 화면은 기존 세대를 유지한다(**한 화면 안에서 두 세대를 섞지 않는다**).
