...
- **2차(세분) 필터**: 세분 카테고리가 2개 이상인 그룹(텐트/매트/배낭/의류/가구/랜턴/조리/기타)을 선택하면 1차 칩 행 아래에 **세분 칩 행**이 나타난다 — `전체` + 그 그룹의 세분 카테고리(한글 라벨, DM-4). 단일 선택, 기본 `전체`, 재탭 해제 시 `전체` 복귀. 세분 선택 시 이미 로드된 그룹 목록을 **클라이언트에서 `category === 세분키`로 필터**한다(추가 쿼리 없음). 1차 필터 변경 시 세분 선택은 초기화. 세분이 1개뿐인 그룹(침낭/전자기기/음식)은 세분 행을 렌더하지 않는다.
