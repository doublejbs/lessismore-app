...
- **비공개 원칙**: 사용자 업로드 이미지는 **로그인한 업로더 본인의 화면에만** 나타난다. 공유 배낭 링크([BagDetail.md](BagDetail.md) BD-7)·박지 후기 첨부 배낭([CampSite.md](CampSite.md) CS-8)·배낭 공유 이미지([BagShare.md](BagShare.md)) 등 **제3자가 보는 표면에서는 렌더하지 않는다.** 이 원칙이 깨지면 2026-07-28에 우려한 재유입 경로가 그대로 열린다.
  ...
  - **`[미해결]` 앱 밖 표면**: 공유 링크(`{WEB_BASE_URL}/bag-share/{bagId}`)를 실제로 여는 웹 랜딩은 **별도 레포**(`lessismore`)이고 Firestore를 직접 읽는다. 이 앱의 조회 차단이 거기엔 적용되지 않으므로, **그쪽에서도 장비 `imageUrl`을 렌더하지 않는지 확인해야 비공개 원칙이 완성된다.** 2026-07-28 제거 작업의 잔여 항목으로도 남아 있었다.
- **표시 지점은 "본인이 보는 장비 행" 전부**다. 공용 행 문법(`LiquidMetricRow` + `GearThumbnailView`)을 쓰는 화면이 자동으로 포함되므로 화면을 하나씩 열거하는 대신 규칙으로 정의한다 — 창고 목록([Warehouse.md](Warehouse.md) WH-1), 배낭 상세 장비 행([BagDetail.md](BagDetail.md) BD-1), 배낭 편집(BD-4)·패킹([Packing.md](Packing.md))·사용 기록(BD-5)의 장비 행, 검색·탐색의 **목록 행**([Search.md](Search.md) SR), 그리고 장비 상세([GearDetail.md](GearDetail.md) GD-1·GD-13).
  - **행과 카드를 구분한다.** 피드의 2컬럼 **카드**(FD-2)는 이미지 없는 텍스트 카드를 유지한다 — 무게가 카드의 시각 앵커인 구조라 썸네일이 들어가면 위계가 무너진다. 검색 결과는 같은 **카드**(FD-2 재사용)라 썸네일이 없고, 탐색 목록(SR-7)·브랜드 목록은 같은 **행**을 쓰므로 썸네일이 나온다(2026-08-11: 그 행도 Liquid `LiquidMetricRow`로 이식돼 창고·배낭 상세와 한 문법이다).
...
**표시 이름의 대괄호 접미는 떼고 보여준다** (2026-08-11 디자인 리뷰 · `model/gear/GearDisplayName.ts`).
카탈로그 크롤 원본의 `name`·`nameKorean`에는 재고·판매 상태 주석이 이름 끝에 붙어 있다
(`마운틴 스패츠 라지 [품절]`). 쇼핑 문맥에서는 뜻이 있었지만 내 창고·내 배낭 목록에서는 이미 가진
물건에 남의 재고 상태가 따라다니는 셈이라 읽을 이유가 없다.

- **떼는 곳은 `Gear.getDisplayName()` 하나다.** 화면마다 각자 떼면 창고에서 사라진 접미가 검색
  결과에는 남는다. 표시 게터를 쓰는 창고([Warehouse.md](Warehouse.md) WH-1)·배낭·검색·상세가
  자동으로 같은 이름을 쓴다.
- **저장값과 캐논컬 값은 건드리지 않는다** — `name`은 정렬(`orderBy('name')`)·중복 키·편집 프리필의
  기준이라 문서 그대로여야 하고(위 표), 마이그레이션도 하지 않는다. 표시 시점 정규화이며
  `colorKorean` 폴백의 첫 글자 대문자 처리와 같은 층이다.
- 대상은 **이름 끝에 붙은 대괄호 묶음**뿐이다(`[품절] [재입고]`처럼 연달아 붙어도 함께 뗀다). 앞에
  오는 `[단독]`이나 가운데 대괄호는 이름·규격의 일부로 읽히므로 남긴다. 이름이 통째로 대괄호면
  그것이 곧 이름이라 원본을 남긴다(빈 이름을 내놓지 않는다).
...
### DM-26 무게 표기 규칙 (2026-08-11 사용자 결정)

무게 표기의 정본은 `model/gear/WeightFormat.ts`이며, **표시 문자열을 만드는 곳은 이 파일뿐이다**. 화면마다 `toFixed()`를 부르던 결과로 같은 값이 `0.00kg`(배낭 편집 헤더)·`3.23kg`(배낭 목록)·`8.4kg`(운동 기록)·`6.61KG`(공유 카드)으로 갈렸다.

- **장비 개별 무게는 항상 g** — `55g` `7200g`. 창고 목록의 세로 비교축이라 값 크기에 따라 단위를 바꾸지 않고, 천 단위 구분자도 넣지 않는다.
- **배낭·창고 합계는 kg, 소수 한 자리**이며 뒤의 `.0`은 뗀다 — `0kg` `3.2kg` `10kg` `44.7kg`.
- **숫자와 단위는 한 덩어리로 조판한다.** 단위를 다른 크기로 앉히는 히어로 수치는 같은 텍스트 블록 안에서 잇고(`formatBagWeightValue` + 단위 스팬), `kg 총 무게`처럼 단위와 라벨을 묶은 **별도 줄을 두지 않는다**.
- **계산과 표시를 가른다.** 모델은 저장 단위(g)만 노출하고(`getWeightGram`·`getPackedWeightGram`·`getTotalWeightGram`·`getUsedWeightGram`·`totalWeightGram`) 합산·차감·정렬·평균을 g 정수로 한다 — kg로 미리 반올림한 값끼리 계산하면 정렬에서 동률이 뭉개지고(BAG-6) 델타 배지에 오차가 실린다.
- **무게 미입력 판정은 `hasWeight()`** — 0·빈 문자열·`null`·숫자 아님을 모두 미입력으로 본다(장비 0g은 실제 무게가 아니라 아직 안 적은 값). 목록 행은 미입력일 때 수치 프롭을 빼지 않고 `null`을 넘겨 그 자리에 `무게 미입력`을 놓는다 — 칸이 사라지면 우측 정렬축이 무너진다. 단 **배낭 총 무게 0은 빈 배낭**이므로 이 판정을 쓰지 않는다.
- **kg 스냅샷 예외**: 박지 후기의 `bagWeight`(DM-20)만 kg 문자열로 저장한다. 새 값은 `formatBagWeightValue(g)`로 쓰고, 옛 문서에 남은 `8.40` 같은 두 자리 값은 표시 시점에 `formatBagWeightFromKilograms()`가 정규화한다.
- **대문자 `KG` 예외**: 공유 카드 내보내기 캔버스([BagShare.md](BagShare.md) BS-4·BS-8)만 대문자 단위와 공백 구분을 쓴다 — 자리수는 위 규칙과 같다.
