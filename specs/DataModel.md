...
| `imageUrl` | string? | **레거시 대표 사진 URL** — 박지 상세(CS-3 ⑤)·웹 랜딩(CS-7)이 읽는다. 고캠핑 스냅샷의 `firstImageUrl`과 큐레이션 수동 입력값이 들어 있다. **일괄 백필 대상이 아니다**(아래 "대표 사진" ①) — 라이선스 유형을 확인할 수 없는 값이 섞여 있어 표시에 조건이 붙는다([CampSite.md](CampSite.md) CS-10) |
| `source` | string | `gocamping` / `knps` / `curated` — 박지 **레코드**의 출처 표기용 |
...
**대표 사진 — 이 컬렉션은 갖지 않는다 (2026-08-17 확정)** `[기획]`

① **추천 카드용 사진은 `camp-spot`이 아니라 `feed-content`(DM-27)에 둔다.** **필드 계약은 DM-27 "카드 대표 사진"** 이 갖고, **소스·라이선스 계약은 아래 ②**(두 컬렉션이 공유), **후보 선정·표기 규칙은 [CampSite.md](CampSite.md) CS-10**이 갖는다.

- **근거는 실측 매칭률이다**(2026-08-17, `camp-spot` 532건 중 앞 40건 표본을 KorService2로 매칭):
  - 이름 매칭 + 공공누리 제1유형(`Type1`) + 5km 이내: **6/40 (15%)**
  - 후보 없음: **17/40 (42.5%)**
  - 나머지는 좌표 근접 후보뿐인데 내용이 `구미캠핑장 → 금오시장`, `약수골캠프촌 → 전기필불망비`, `자양보오토캠핑장 → 산청 조식 유적`, `카누글램핑펜션 → 토마토조각공원` 이다. **박지 사진이 아니다.**
- 그래서 **`camp-spot`에 사진 필드 세트를 두지 않는다**: 532건 중 85%가 빈 필드로 남고, 필드가 존재한다는 사실 자체가 "박지에는 대표 사진이 있다"는 **틀린 기대를 코드와 화면에 심는다**(그 기대 위에 밴드·레이아웃을 짜면 대다수 케이스가 예외가 된다).
- 반대로 홈 추천은 **한 번에 최대 5개**이고 운영자가 손으로 고른다 — 사람이 확인해 사진을 붙일 수 있는 규모다. **사람이 고르는 값은 사람이 쓰는 컬렉션에 있어야 한다**: `feed-content`는 웹 CMS가 쓰는 컬렉션이고(DM-27), `camp-spot`은 **클라이언트 쓰기 금지 + 스크립트 전용**이라 발행 흐름에서 사진을 붙일 경로가 애초에 없다.
- **"같은 박지가 홈과 상세에서 다른 사진"은 결함이 아니라 정의다.** 추천 카드의 사진은 `summary`와 같은 성격의 **그 발행의 편집물**(운영자가 이 박지를 소개하며 고른 한 장)이고, `camp-spot.imageUrl`은 **카탈로그의 속성**이다. 두 값이 같을 필요가 없다.
- **기존 `camp-spot.imageUrl`은 그대로 둔다** — 박지 상세·웹 랜딩이 계속 읽는다. 다만 **TourAPI 일괄 백필을 하지 않고**, 라이선스 유형을 확인할 수 없는 값의 표시 조건은 [CampSite.md](CampSite.md) CS-10이 정한다.
- **재시드 유실 방어**(`scripts/backfill-camp-spot-city.mjs`가 남긴 `city`·`region` 되돌아감 이력과 같은 종류):
  - `feed-content`는 `seed-camp-spots.mjs`가 건드리지 않는 컬렉션이라 **이 결정으로 재시드 유실 위험 자체가 사라진다.**
  - `camp-spot` 쪽에도 규칙을 남긴다: **시드는 기존 문서의 이미지 필드를 덮어쓰지 않는다.** 지금 시드는 `setDoc(..., { merge: true })` + "빈 값은 키를 넣지 않는다"(`withOptional`)라 없는 키를 지우지는 않지만, 고캠핑 문서는 재시드마다 페이로드에 `imageUrl`(=`firstImageUrl`)을 실으므로 **사람이 손으로 넣은 값을 되돌린다.** 사람이 확인해 넣은 `imageUrl`이 있는 문서는 그 키를 페이로드에서 빼야 한다(또는 `scripts/camp-spots-curated.json`에 함께 기록한다 — city 백필이 세운 관례).

② **TourAPI 계약** — 소스는 **한국관광공사 TourAPI**(공공데이터포털)의 국문 관광정보 서비스 **KorService2**다. 아래 계약은 `feed-content` 사진(DM-27)과 향후 `camp-spot` 수동 입력이 함께 쓴다.

| 항목 | 값 |
| --- | --- |
| End Point | `https://apis.data.go.kr/B551011/KorService2` |
| 오퍼레이션 | `searchKeyword2`(키워드 검색) · `detailImage2`(`contentId` + `imageYN=Y` → **이미지 갤러리**) · `locationBasedList2`(좌표 + `radius`, 응답에 **`dist`(m)** 포함) · `detailCommon2`(단건 상세) |
| 공통 파라미터 | `serviceKey`, `MobileOS`, `MobileApp`, `_type=json`, `numOfRows` |
| 이미지 필드 | 항목: `firstimage`(첫 사진) / `firstimage2`(썸네일) · 갤러리: `originimgurl`(원본) / `smallimageurl` |
| 좌표 필드 | `mapx`(경도) / `mapy`(위도) |
| 라이선스 필드 | **`cpyrhtDivCd`** — **항목과 개별 이미지가 각각 갖고, 값이 다르다**(아래) |

같은 키로 `PhotoGalleryService1`도 열리지만 **이번 계약은 KorService2 기준**이다(2026-08-17 실호출 확인).

**`firstimage`를 그대로 대표 사진으로 쓰지 않는다 — `detailImage2` 갤러리까지 받아 사람이 고른다.** `firstimage`는 그 장소의 **첫 사진일 뿐**이라 대표성이 보장되지 않는다. 실측(2026-08-17): `안반데기`의 `firstimage`는 **나무 안내판 사진**이었고, `detailImage2`로 갤러리를 받으면 7장이 더 나와 그중 고랭지 밭·풍력발전기가 담긴 광각 풍경이 있다. 파이프라인은 **`searchKeyword2`로 `contentid` 확보 → `detailImage2`로 갤러리 조회 → 이미지 단위 `Type1` 필터 → 사람이 고름** 이다(CS-10). `firstimage`도 후보 풀에 남는다 — 실제로 확정 3건 중 둘은 `firstimage`가 채택됐다(DM-27). 배제하는 것은 **확인 없이 첫 사진을 쓰는 것**이다.

**웹 크롤링은 쓰지 않는다** — §1 장비 이미지 정책이 리스크를 "남의 저작물"과 "재유포 경로"로 짚은 그 기준을 그대로 적용하면, 브랜드·블로그 사진을 긁어 오는 경로는 소스가 박지로 바뀌어도 같은 두 조건에 걸린다. 공공누리 저작물은 권리자가 명시적으로 이용을 허락한 것이라 두 조건 어디에도 걸리지 않는다 — 단 **허락의 범위가 유형마다 다르므로 유형 확인이 이 계약의 핵심 제약**이다(아래). (§1 "범위 밖" 항목이 박지 사진을 장비 이미지 정책 밖으로 둔 것과 별개로, **박지 사진을 쓸 수 있는 근거는 라이선스**다.)

**라이선스는 `cpyrhtDivCd`가 가른다 — 항목이 아니라 이미지 단위로 확인하고, `Type1`만 채택한다.** 이것이 이 계약의 핵심 제약이다.

- ★ **항목의 `cpyrhtDivCd`로 판단하면 위반이다.** 항목(item)과 `detailImage2`의 개별 이미지가 각각 `cpyrhtDivCd`를 갖고 **값이 다르다.** 실측(2026-08-17): `하화도 꽃섬길`은 **항목이 `Type1`인데 갤러리 10장은 전부 `Type3`** 였다. 장소가 제1유형이라고 그 장소의 사진이 제1유형인 것이 아니다.
- 따라서 **채택하는 그 이미지의 `cpyrhtDivCd`를 확인해 함께 기록한다**(`imageLicense`). 항목 값은 후보를 좁히는 참고조차 되지 않으므로 쓰지 않는다.

| `cpyrhtDivCd` | 공공누리 유형 | 채택 | 이유 |
| --- | --- | --- | --- |
| `Type1` | 제1유형 — 출처표시, **변경 허락** | **채택** | 크롭이 허용된다 |
| `Type2` | 제2유형 — 출처표시, **상업적 이용금지** | **제외** | 앱이 상업 서비스다(제휴 링크 노출) |
| `Type3` | 제3유형 — 출처표시, **변경금지** | **제외** | 카드 밴드가 `cover`로 **크롭**한다 = 변경 |
| 값 없음·미지 | — | **제외** | 허락 범위를 확인할 수 없는 저작물은 쓰지 않는다 |

- **`Type3` 제외의 근거는 크롭이다.** 카드 밴드(HM-11)는 고정 높이 110pt에 `contentFit='cover'`로 넣으므로 원본 비율과 다른 영역이 잘린다 — 변경금지 저작물에 할 수 없는 처리다. 밴드를 원본 비율로 그리게 바꾸면 `Type3`도 검토할 수 있지만, 그러면 카드 높이가 사진마다 달라져 캐러셀 문법이 깨진다(HM-11). 현 결론: `Type1`만.
- **`Type2` 제외의 근거는 서비스 성격이다** — 앱이 제휴 링크로 수익을 내는 상업 서비스라 "상업적 이용금지" 조건과 양립하지 않는다. 크롭을 걷어내도 이 유형은 되살아나지 않는다.
- **필터는 스크립트의 필수 단계**다([CampSite.md](CampSite.md) CS-10). `Type1`이 아닌 후보는 사진이 아무리 좋아도 **후보 목록에 올리지 않는다** — 사람이 고르는 화면에 올라오면 언젠가 채택된다.
- **실측(2026-08-17)**: 발행된 추천 3건 중 `구봉도`는 이름이 정확히 일치하는 레코드가 **`Type3`라 탈락**했다. 유형 필터가 없으면 변경금지 저작물을 그대로 크롭해 실었을 케이스다.
- **URL은 `http` → `https`로 치환해 저장한다.** `firstimage`·`originimgurl`이 `http://tong.visitkorea.or.kr/...`로 오는데, iOS ATS와 웹 mixed content가 평문 HTTP 이미지를 막아 밴드가 통째로 폴백된다. **같은 경로가 https로 200을 준다**(2026-08-17 실측). 치환 후 200을 확인하지 못한 URL은 **저장하지 않는다**(깨진 값을 넣지 않는다).
- **사진 우하단에 한국관광공사 워터마크가 박혀 있다**(2026-08-17 실측). 출처가 사진 자체에 들어 있는 셈이지만, **표기는 워터마크에 의존하지 않는다** — 카드 밴드는 `cover`로 크롭해 하단이 잘릴 수 있어 워터마크가 사라질 수 있다. 그래서 **캡션 표기가 정본**이고, 워터마크가 잘리는 것은 허용한다. 반대로 **워터마크를 일부러 지우거나 가리는 처리(블러·덮기·리터칭)는 금지**한다 — 그건 출처 표시를 능동적으로 제거하는 것이다. 표기가 반드시 화면에 함께 보이는지는 수용 기준으로 확인한다([CampSite.md](CampSite.md) CS-10, [Home.md](Home.md) HM-11).
- 필드별 매핑(`originimgurl` → `imageUrl` 등)은 필드를 소유한 **DM-27 "카드 대표 사진"** 표에 있다.

③ **저장 방식은 제공기관 URL 직접 참조다** — 우리 Storage로 복사하지 않는다.

- 공공누리 제1유형은 **저장·변형을 허용하므로 사본을 두는 것도 적법하다.** 즉 이것은 라이선스 제약이 아니라 **복잡도 판단**이며, [Bag.md](Bag.md) BAG-1 지도 밴드의 "서버 저장 금지"(NCP 약관상 **금지**)와는 성격이 다르다. 두 규칙을 같은 것으로 읽지 말 것.
- 직접 참조를 고른 이유: (a) `tong.visitkorea.or.kr` URL은 서명·만료 쿼리가 없는 **정적 CMS 경로**라 프리사인 만료 위험이 없다(`.../cms/resource/{nn}/{id}_image2_1.jpg` 형태), (b) 사본을 두면 Storage 경로(DM-9)·공개 읽기 권한·교체 시 이전 파일 삭제까지 따라오는데, 지금 Storage 보안 규칙은 §1 `[운영]`에 따라 **본인 경로만 허용하도록 좁히는 방향**이라 공개 읽기 이미지 경로를 새로 여는 것과 충돌한다, (c) 사진이 없거나 깨져도 화면은 폴백으로 정상 동작한다(HM-11·CS-10) — 가용성 요구가 낮다, (d) **대상이 소수**다(발행 5건 규모) — 사본 관리 비용을 정당화할 물량이 아니다.
- **위험도 함께 적는다**: 이 호스트는 우리 통제 밖이다. 서명 URL이 아니라 **핫링크 차단을 언제든 켤 수 있고**(Referer·UA 기반), CMS 재구성으로 경로가 바뀌면 조용히 404가 된다. 그때 화면은 밴드 없는 폴백으로 떨어져 **깨지지는 않지만 사진이 사라진다** — 즉 이 결정의 리스크는 "장애"가 아니라 "조용한 소실"이며, 그래서 재검증이 필요하다(§8).
- **Storage 사본으로 승격할 조건**(하나라도 충족하면 후속 기획):
  1. 재검증에서 **로드 실패(404·경로 변경)가 5%를 넘을 때**.
  2. `tong.visitkorea.or.kr`가 **핫링크 차단**·인증 요구·https 미지원으로 앱에서 로드가 막힐 때.
  3. 응답이 느려 카드 밴드가 체감상 비어 보일 때(측정 근거 필요).
- 승격하면 DM-9에 `/feed-content/{contentId}/{fileName}` 경로를 더하고 **출처 필드는 그대로 유지한다** — 공공누리 표기 의무는 사본에도 따라온다. 공공누리 제1유형이 저장·변형을 허용하므로 사본 자체는 언제든 적법하다.

④ **매칭은 발행 시점 1회이고, 자동 채택이 없다** — 앱은 런타임에 TourAPI를 호출하지 않는다. 스크립트는 **후보만 제시**하고 채택은 사람이 한다. **좌표 근접 폴백은 폐기했다**(실측 근거·규칙은 [CampSite.md](CampSite.md) CS-10).

⑤ **TourAPI 키는 앱 번들에 넣지 않는다.** 런타임 호출이 없으므로 클라이언트가 키를 가질 이유가 없다. **저장소 루트 `.env`의 `TOUR_API_KEY`**(gitignore 대상)를 스크립트에서만 읽고, `EXPO_PUBLIC_` 접두사를 **쓰지 않는다**(그 접두사는 값이 번들에 인라인된다). 기존 `GOCAMPING_KEY` 관례와 같다 — 같은 포털 키를 쓸 수 있어도 변수를 분리해 둔다(고캠핑 스냅샷과 사진 후보 조회는 독립 실행이다).
...
**카드 대표 사진 (2026-08-17 확정)** `[기획]`

홈 추천 카드의 사진 밴드([Home.md](Home.md) HM-11)가 읽는 사진은 **이 컬렉션이 갖는다**(`camp-spot`이 아니다 — 근거는 DM-17 "대표 사진" ①: 실측 자동 매칭률 15%, 사람이 고르는 값은 CMS가 쓰는 컬렉션에). 소스·라이선스·URL 규칙은 DM-17 "대표 사진" ②가 정본이다.

| 필드 | 타입 | 비고 |
| --- | --- | --- |
| `imageUrl` | string? | 대표 사진 URL — 후보는 `detailImage2` 갤러리(`originimgurl`)와 항목 `firstimage` **둘 다**이고, **사람이 내용을 보고 고른 한 장**이다(`firstimage`를 무조건 쓰지 않는다 — 대표성이 없을 때가 많다, DM-17 ②). **`http` → `https` 치환 후 저장**하고 200을 확인한다. 우리 Storage 사본이 아니라 **제공기관 URL 직접 참조**(DM-17 ③) |
| `imageSource` | string? | 제공기관명 — `한국관광공사`. 구조화된 값. `imageUrl`이 있으면 **필수** |
| `imageLicense` | string? | **채택한 그 이미지의** `cpyrhtDivCd` → string enum `ImageLicense`(`model/feed/ImageLicense.ts`). **유효값은 `kogl-type1`(공공누리 제1유형) 하나**다 — 제2·제3유형·값 없음은 애초에 저장하지 않는다. **항목 수준 값으로 채우면 위반**이다(DM-17 ②). `imageUrl`이 있으면 **필수** |
| `imageAttribution` | string? | 화면이 **그대로 내는 표기 문구** — `사진: 한국관광공사 (공공누리 제1유형)`. 문구를 뷰 코드가 조립하지 않게 저장 시점에 확정한다(소스·유형이 늘어도 화면을 고치지 않는다). `imageSource`·`imageLicense`와 겹치는 것은 **의도된 중복**이며, 없으면 그 둘로 조립하는 폴백을 둔다. `imageUrl`이 있으면 **필수** |
| `imageContentId` | string? | TourAPI `contentid` — 재조회·재검증·매칭 근거 추적용. 개별 이미지 식별자는 URL 경로에 들어 있다 |
| `imageUpdatedAt` | string? | ISO 8601 — 사진 필드 세트를 채운 시각. 재검증 기준 |

- **사진 필드는 세트로만 쓴다** — `imageUrl`을 쓰는 쓰기는 `imageSource`·`imageLicense`·`imageAttribution`·`imageContentId`·`imageUpdatedAt`을 **함께** 쓴다. `imageUrl`만 갱신해 표기 문구가 다른 사진을 가리키는 상태(=출처 표시 위반)를 만들지 않는다.
- **`imageUrl`은 필수가 아니다 — 없는 것이 정상 상태다.** 실측 자동 매칭률이 15%이고 나머지는 사람이 확인해야 하므로, 사진 없는 발행이 다수일 수 있다. 카드는 사진 없이 성립한다(HM-11 폴백).
- **쓰기 경로는 웹 CMS다**(DM-27 본문 — 쓰기 admin 전용). 이 레포의 스크립트는 **후보를 제시만** 하고 Firestore에 쓰지 않는다([CampSite.md](CampSite.md) CS-10). CMS에 사진 URL 입력 칸과 후보 붙여넣기를 만드는 일은 **별도 레포(lessismore 웹) 후속 작업**이다.
- **사람이 검증해 확정한 값 3건**(2026-08-17 — 전부 이미지 단위 `Type1` · https 200 · 내용 육안 검증). 후속 시드·발행이 **이 값을 그대로 쓴다**:

  | 박지 | `imageContentId` | `imageUrl` | 내용 |
  | --- | --- | --- | --- |
  | 안반데기 | `2714439` | `https://tong.visitkorea.or.kr/cms/resource/56/3537656_image2_1.jpg` | 고랭지 밭·풍력발전기 광각 (`detailImage2` 갤러리에서 고름 — `firstimage`는 안내판이었다) |
  | 하화도 | `2381140` | `https://tong.visitkorea.or.kr/cms/resource/35/3018735_image2_1.jpg` | 꽃섬길 데크·바다 |
  | 구봉도 | `2761294` | `https://tong.visitkorea.or.kr/cms/resource/88/3563488_image2_1.jpg` | 개미허리아치교 해질녘 |
...
- 필요한 트리거: 그룹 문서 삭제 시 하위 컬렉션·Storage GPX·**전 멤버 역인덱스** 정리, 그룹 정보 수정 시 멤버 역인덱스 갱신, `memberIds` 변경 시 역인덱스 생성·삭제, Auth 삭제 시 소속 그룹 정리(방장이면 해산).
