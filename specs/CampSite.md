...
- **즐겨찾기 리스트 시트**: 유형 칩 1행의 `전체` 앞에 **★ 칩**을 둔다(별 아이콘 전용, `accessibilityLabel='즐겨찾기 목록'`). 탭 시 즐겨찾기한 박지 **리스트를 바텀 시트**로 연다 — 지도를 필터링하지 않는다. 리스트 항목 = 박지명·유형 배지·지역, 탭하면 **시트를 닫고 그 박지로 카메라를 이동한 뒤 상세를 연다**(지도 탭) 또는 **그 박지를 선택**한다(여행지 선택기, DST-3). 즐겨찾기 목록은 로드된 활성 박지와 즐겨찾기 id를 조인해 만든다(삭제·비활성 박지는 자연히 빠진다). 비로그인이 ★ 칩을 탭하면 로그인 안내 후 중단하고, 로그인했으나 0건이면 시트에 `아직 즐겨찾기한 박지가 없어요` 빈 상태를 표시한다.
