...
- **선택 마커 강조 + 애니메이션**: 마커를 탭해 요약 카드(CS-4)가 뜨면 **그 마커를 선택 상태로 강조**한다.
...
- 마커 탭 → 하단 요약 카드(이름·유형 배지·지역·**설명**·시설 아이콘 요약). 지도 빈 곳 탭 → 카드 닫힘.
  - **설명**은 지역 아래에 **최대 2줄**(말줄임)로 표시한다 — 카드가 지도를 과하게 가리지 않게 하고, 전문은 상세(CS-3)에서 본다. `description`이 비어 있으면 렌더하지 않는다.
