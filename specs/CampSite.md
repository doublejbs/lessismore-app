...
- **선택 마커 강조**: 마커를 탭해 요약 카드(CS-4)가 뜨면 **그 마커를 선택 상태로 강조**한다 — 앱의 활성=검정 언어에 맞춰 **검정 강조 링 + 흰 테두리 + 유형색 원**을 키워(비선택 20 → 선택 32) 그림자와 함께 팝시키고, 캡션도 살짝 키워 위로 올린다. 유형색은 링 안에 유지해 유형 정보를 잃지 않는다. 선택 상태는 모델(`campSiteMap.getSelectedSpot`)이 단일 소스이며, 마커 레이어(observer)가 그 값을 읽어 강조한다(배낭 여행지 피커 DST-3은 자체 선택 상태를 `selectedSpotId`로 넘긴다).
