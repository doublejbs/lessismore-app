...
- 지도 상단에 **필터 칩 한 행**(가로 스크롤, 공용 `CategoryChipView` 톤): `전체 · [●노지] [●대피소] [●야영장] | [#산] [#해변] … [#숲]` 순.
  - **유형 칩**(단일 선택)에는 라벨 앞에 **마커 색 도트**(노지=파랑/대피소=초록/야영장=골드)를 표시해 칩이 지도 마커 색 범례를 겸한다. 선택(검정 채움) 상태에서는 도트에 흰 테두리를 둘러 시인성을 유지한다.
  - **태그 칩**은 `#` 접두 라벨(`#해변` — 상세 화면 태그 칩과 일관)로 유형과 축을 구분하며, 별도 "전체" 없이 **선택된 칩을 다시 탭하면 해제**(토글)된다. 유형과 태그 사이에는 세로 구분선을 둔다.
  ...
  - **선택 칩 시인성**: 스크롤로 가려진 칩을 선택하는 경우를 포함해, 선택 시 해당 칩이 보이도록 행을 자동 스크롤한다.
