...
- 마커 탭 → **박지 상세(CS-3)를 하단 바텀 시트로** 연다(구글 지도 동작). 별도 요약 카드·요약 시트는 두지 않는다 — 상세가 곧 시트 내용이라 "자세히 보기" 같은 중간 단계가 없다.
  ...
  - **높이·확장**: `sheetAllowedDetents: [0.2, 0.4, 1]` — **기본은 40%**(`sheetInitialDetentIndex: 1`)이고, 위아래로 끌어 20% · 40% · 최대로 전환한다(네이티브 시트 기본 동작). 핸들바(`sheetGrabberVisible`) 노출, 모서리 radius 20.
  - **최소(20%)는 peek 상태** — 이 높이(약 163pt)에는 헤더·탭 바·고정 CTA(합 약 224pt)가 다 들어가지 않아 그대로 그리면 **CTA가 잘린다**. 그래서 시트 높이가 임계값(260pt) 미만이면 **헤더(이름·액션·배지·지역)만 남기고 탭 바·탭 콘텐츠·CTA를 접는다**. 지도를 보는 상태라 그 편이 의도에도 맞다.
...
- **탭 구조**(구글 지도 장소 시트와 동일한 얼개): 시트는 **고정 영역 + 탭 콘텐츠 + 고정 CTA**로 나뉜다.
  - **고정(스크롤되지 않음)**: ① **헤더 행 — 박지 이름 + 액션 아이콘(위치로 이동 / 공유 / 네이버 지도 / 닫기)을 한 줄에** 둔다(구글 지도 장소 시트와 동일). 아이콘만 따로 한 줄을 쓰면 가운데가 빈 채 세로를 낭비한다 — 60% 시트에서 콘텐츠 영역이 좁아 아깝다. 이름은 1줄 말줄임. ② 유형 배지 · 지역 ③ **탭 바**.
  - **탭 바**: `개요` · `날씨` · `후기` — 가로 스크롤 없이 3개가 들어간다. 선택 탭은 라벨을 진하게 + 하단 인디케이터(구글 지도 톤). 기본 선택은 `개요`. 탭은 44pt 터치 타깃과 `accessibilityRole='tab'`을 지킨다.
  - **탭 콘텐츠**: 탭마다 독립 ScrollView. 탭 전환 시 각 탭의 스크롤 위치는 유지하지 않는다(단순화).
  - **고정 CTA**: `배낭 여행지로 설정`(CS-5)은 **탭과 무관하게 하단에 고정**한다 — 박지 단위의 주 액션이라 어느 탭에서도 닿아야 한다.
- **개요 탭**: 대표 사진(있으면), 태그 칩(`#해변` 형태, 비인터랙티브), 주의·규제 고지(CS-4), 설명, 시설(화장실/식수/데크/매점 — 있는 것만 아이콘+라벨), 접근 정보(`accessInfo` 자유 텍스트, 예: `주차장에서 도보 40분`), 출처(`source`). 사진이 있으면 경고가 그만큼 아래로 밀리지만, 사진은 박지 파악의 1차 단서라 이 순서를 유지한다 — 대신 CS-4가 경고를 한 덩어리로 줄여 스크롤 없이 닿게 한다.
- **날씨 탭**: 주간 예보(아래 항목 참고).
