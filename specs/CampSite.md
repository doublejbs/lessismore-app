...
- 지도 상단에 **필터 칩 두 행**(공용 `CategoryChipView` 톤) — 축당 한 행씩 담당해 리셋·색 범례가 항상 보이게 한다(두 축을 한 행에 합치면 "전체"가 스크롤 밖으로 사라지고 태그 발견 가능성이 떨어짐 — 디자인 리뷰로 확정).
  - **1행 유형 칩**: `전체 · [●백패킹] [●대피소] [●캠핑장]` — 4칩이 스크롤 없이 화면 안에 들어간다(단일 선택). 라벨 앞 **마커 색 도트**(백패킹=파랑/대피소=초록/캠핑장=골드)가 지도 마커 색 범례를 겸하며, 선택(검정 채움) 상태에서는 도트에 흰 테두리를 둘러 시인성을 유지한다.
...
- **즐겨찾기 리스트 시트**: 유형 칩 1행의 `전체` 앞에 **★ 칩**을 둔다(별 아이콘 전용, `accessibilityLabel='즐겨찾기 목록'`). 탭 시 즐겨찾기한 박지 **리스트를 높이 조절 바텀 시트**로 연다 — 박지 상세 시트(CS-3)와 동일한 formSheet 라우트 방식(`sheetAllowedDetents: [0.2, 0.4, 1]`, `sheetLargestUndimmedDetentIndex: 'last'`로 딤 없이 뒤 지도 조작, `contentStyle` `bottom: 0`). 상세 시트가 아니라 라우트라 지도의 `CampSiteMap`엔 핸드오프로 콜백을 넘긴다.
  ...
  - 즐겨찾기 목록은 로드된 활성 박지와 즐겨찾기 id를 조인해 만든다(삭제·비활성 박지는 자연히 빠진다). 비로그인이 ★ 칩을 탭하면 로그인 안내 후 중단하고, 로그인했으나 0건이면 시트에 `아직 즐겨찾기한 박지가 없어요` 빈 상태를 표시한다.
