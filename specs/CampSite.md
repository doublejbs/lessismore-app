...
- 마커 탭 → **박지 상세(CS-3)를 하단 바텀 시트로** 연다(구글 지도 동작). 별도 요약 카드·요약 시트는 두지 않는다 — 상세가 곧 시트 내용이라 "자세히 보기" 같은 중간 단계가 없다.
  ...
  - **높이**: 화면 높이의 75%로 고정 — 탭 콘텐츠가 충분히 보이면서 뒤 지도도 남는다. 끌어서 확장은 없다(이유는 CS-3의 시트 높이 항목 참고). 핸들바(`sheetGrabberVisible`) 노출, 모서리 radius 20.
...
- **시트 높이는 화면이 직접 지정한다** — `react-native-screens`는 formSheet에서 콘텐츠 래퍼에 `bottom`을 걸지 않아(시트 높이 변경 시 깜빡임 방지) **React 레이아웃 높이가 무제한**이 된다. 그러면 `flex: 1`이 뷰포트를 못 잡아 ScrollView가 콘텐츠 높이만큼 늘어나고 **스크롤이 아예 죽는다**(detent 아래는 잘려서 접근 불가). 하단 고정 요소(CTA·토스트)도 화면 밖에 배치된다.
  - 따라서 `sheetAllowedDetents`는 **`fitToContents`** 로 두고(= 시트가 콘텐츠 높이에 맞춰짐), 래퍼가 루트 View에 **화면 높이 비율로 명시적 height**를 준다. 경계가 생기면 `flex: 1` 체인이 살아나 스크롤·하단 고정 CTA·토스트가 모두 정상 동작한다. 상세는 화면의 **0.75**를 쓴다(헤더·제목·탭 바·고정 CTA를 빼고도 탭 콘텐츠가 충분히 남는 높이).
  - 이 방식은 detent가 하나뿐이라 **끌어서 확장은 없다** — 고정 높이 시트다.
...
- **탭 구조**(구글 지도 장소 시트와 동일한 얼개): 시트는 **고정 영역 + 탭 콘텐츠 + 고정 CTA**로 나뉜다.
  - **고정(스크롤되지 않음)**: ① 헤더 아이콘 행(닫기 / 위치로 이동 / 공유 / 네이버 지도) ② 이름 · 유형 배지 · 지역 ③ **탭 바**.
...
- **개요 탭**: 대표 사진(있으면), 태그 칩(`#해변` 형태, 비인터랙티브), 주의·규제 고지(CS-4), 설명, 시설(화장실/식수/데크/매점 — 있는 것만 아이콘+라벨), 접근 정보(`accessInfo` 자유 텍스트, 예: `주차장에서 도보 40분`), 출처(`source`).
- **날씨 탭**: 주간 예보(아래 항목 참고).
...
- **배치**: 상세의 **`후기` 탭**(CS-3) 안에 유저 후기를 먼저 두고, 그 아래에 외부 콘텐츠 섹션을 **`블로그·영상`**으로 이름을 바꿔 배치한다(두 "후기" 라벨 혼동 방지).
...
- [ ] 마커 탭 → 상세 시트(탭바를 덮고 슬라이드 인, 뒤 지도 조작 가능), 시트 안 스크롤 동작, 닫기/스와이프 → 시트 닫힘 + 마커 강조 해제
