...
- **마커 표시는 줌 수준에 따라 2단계**로 나뉜다(유형 구분 없이 일괄): 줌 임계(위도 델타 ≤ 1.2, 도 단위) 이내로 줌인 시 필터 적용된 마커 전량 표시. 줌아웃 상태에선 **현재 화면(뷰포트) 영역 안의 spots를 대상으로 최대 30개** 표시 — 화면을 위경도 격자(세로 6분할 기준)로 나눠 셀당 1개 우선으로 고르게 분산시키고, 부족분은 순서대로 보충한다. 카메라 이동/줌 변경 시 샘플을 갱신한다. 유형 필터는 양쪽 모두 동일 적용. `[제안]`
...
- **후기 콘텐츠** `[제안]`: 상세 하단(날씨 아래)에 `후기` 섹션을 표시한다. 검색어는 두 소스 모두 `"{박지명} 백패킹"`.
  ...
  - 소스별로 키 미설정·조회 실패·0건이면 해당 리스트만 조용히 생략. 블로그 리스트 하단 `네이버 블로그에서 더 보기` 행은 항상 표시 — `https://search.naver.com/search.naver?where=blog&query={박지명}+백패킹`.
