...
- 지도 상단에 **필터 칩 한 행**(가로 스크롤, 공용 `CategoryChipView` 톤): `전체 · [●야영장] [●대피소] [●노지] | [#산] [#해변] … [#숲]` 순.
  - **유형 칩**(단일 선택)에는 라벨 앞에 **마커 색 도트**(야영장=검정/대피소=회색/노지=주황)를 표시해 칩이 지도 마커 색 범례를 겸한다. 선택(검정 채움) 상태에서는 도트에 흰 테두리를 둘러 시인성을 유지한다.
