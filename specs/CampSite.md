...
- **마커 표시는 줌 수준에 따라 2단계**로 나뉜다(유형 구분 없이 일괄): 줌 임계(위도 델타 ≤ 1.2, 도 단위) 이내로 줌인 시 필터 적용된 마커 전량 표시. 줌아웃 상태에선 **최대 30개로 갯수 제한**해 표시하되, 지역 편중을 피하기 위해 위경도 격자(0.5°) 샘플링으로 고른다(셀당 1개 우선, 부족분은 순서대로 보충). 유형 필터는 양쪽 모두 동일 적용. `[제안]`
