...
- **마커 표시는 줌 수준에 따라 2단계**로 나뉜다(유형 구분 없이 일괄): 줌 임계(위도 델타 ≤ 1.2, 도 단위) 이내로 줌인 시 **현재 화면(뷰포트) 영역 안의** 필터 적용 마커 전량 표시. 줌아웃 상태에선 화면 영역 안의 spots를 대상으로 **최대 30개** 표시 — 화면을 위경도 격자(세로 6분할 기준)로 나눠 셀당 1개 우선으로 고르게 분산시키고, 부족분은 순서대로 보충한다. 카메라 이동/줌 변경 시 표시 대상을 갱신한다. 유형 필터는 양쪽 모두 동일 적용. 어느 줌 단계든 **뷰포트 밖 마커는 렌더하지 않는다**(전체 spots를 전부 네이티브 마커로 올리면 탭 반응이 느려짐). `[제안]`
