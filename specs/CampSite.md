...
- **지도 필터**: 유형 칩 1행의 `전체` 앞에 **★ 칩**을 둔다(별 아이콘 전용, `accessibilityLabel='즐겨찾기만 보기'`). 선택 시 즐겨찾기한 박지만 마커 표시(유형·태그 필터와 AND 결합). 비로그인이거나 즐겨찾기 0건이어도 칩은 노출하되, 탭 시 비로그인은 로그인 안내, 0건은 토스트 `즐겨찾기한 박지가 없어요`를 띄우고 선택하지 않는다.
