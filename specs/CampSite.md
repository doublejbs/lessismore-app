...
- 유형 칩 아래에 **태그 필터 칩 행**(전체 + `CampSiteTag` 7종: 산/해변/계곡/섬/호수/초원/숲, 단일 선택)을 둔다. 유형 필터와 **AND 결합**이며, 태그 선택 시 해당 태그가 없는 spot(태그 미부여 포함)은 마커·검색과 무관하게 마커 표시에서 제외된다. 검색(CS-6)은 태그 필터와 독립.
