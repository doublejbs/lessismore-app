...
- 마커 탭 → **박지 상세(CS-3)를 하단 바텀 시트로** 연다(구글 지도 동작). 별도 요약 카드·요약 시트는 두지 않는다 — 상세가 곧 시트 내용이라 "자세히 보기" 같은 중간 단계가 없다.
  ...
  - **높이·확장**: `sheetAllowedDetents: [0.5, 1]` — 절반 높이로 떠서 이름·사진·요약이 먼저 보이고, 위로 끌면 전체로 확장된다. 핸들바(`sheetGrabberVisible`) 노출, 모서리 radius 20.
...
- [ ] 마커 탭 → 상세 시트(탭바를 덮고 슬라이드 인, 절반 높이 → 끌어서 확장, 뒤 지도 조작 가능), 닫기/스와이프 → 시트 닫힘 + 마커 강조 해제
