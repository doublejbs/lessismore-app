...
app/(tabs)/map.tsx → CampSiteMapWrapper → CampSiteMapView (네이버 지도)
  ...
  ├─ 박지 마커 × n (유형별 색/아이콘)
  ├─ 현재 위치 버튼
  ├─ 마커 탭 → /camp-site-summary (하단 요약 바텀 시트)
  └─ 시트의 자세히 보기 → /camp-site/{id} → CampSiteDetailView
```
...
- **선택 마커 강조 + 애니메이션**: 마커를 탭해 요약 시트가 뜨면 **그 마커를 선택 상태로 강조**한다.
...
- 마커 탭 → **하단 요약 바텀 시트**(구글 지도 동작). 지도 위 플로팅 카드가 아니라 루트 라우트 `/camp-site-summary`의 네이티브 `formSheet`이며, 다음을 만족한다.
  - **탭바를 가린다**: 시트가 탭바 위로 슬라이드 인/아웃하며 네이티브 시트 애니메이션이 그 전환을 담당한다(iOS `NativeTabs`는 탭바 전체를 숨기는 API가 없어 루트 레벨 시트로 덮는 방식을 쓴다).
  - **뒤 지도는 계속 조작 가능**: `sheetLargestUndimmedDetentIndex: 'last'`로 딤을 없애 시트가 떠 있는 동안에도 지도를 패닝·줌할 수 있다. 높이는 `fitToContents`, 핸들바(`sheetGrabberVisible`) 노출, 모서리 radius 20.
  - **닫힘**: 아래로 스와이프 또는 시트 우상단 **닫기(X)** → 시트가 닫히고 마커 선택 강조가 해제된다. 지도 빈 곳 탭도 선택을 해제한다.
  - **시트는 항상 1개**: 시트가 떠 있는 동안에도 뒤 지도를 조작할 수 있어 다른 마커를 탭할 수 있다. 이때 **기존 시트를 닫고 새 시트를 연다** — 위로 쌓지 않는다. 닫히는 기존 시트의 선택 해제가 새로 선택된 박지의 강조를 지우지 않도록, 선택 해제는 **자기 박지가 아직 선택 상태일 때만** 수행한다.
  - **내용**: 상단 행에 이름 + 상세 화면과 동일한 기능 버튼 **공유**(CS-7)·**네이버 지도에서 열기**(CS-3) + **닫기**. 그 아래 유형 배지·지역·**설명**·시설 아이콘 요약, 하단에 **위치로 이동** / **자세히 보기**.
  - **설명**은 **최대 3줄**(말줄임) — 전문은 상세(CS-3)에서 본다. `description`이 비어 있으면 렌더하지 않는다.
  - 공유·네이버 지도 열기는 상세와 **동일한 구현을 공유**한다(`CampSiteActions`) — 계측 이벤트도 상세와 같다.
- 시트에 **위치로 이동** 버튼을 둔다 — 시트가 열린 채 지도를 움직였다가 다시 해당 박지 위치로 카메라를 되돌린다(줌인 이동, 검색 결과 선택과 동일한 줌 레벨).

### CS-3 박지 상세

**수용 기준**

- 요약 시트의 **자세히 보기** 탭 → `/camp-site/{id}` 상세 화면. 박지 데이터를 불러오는 동안 빈 화면 대신 **로딩 인디케이터**(중앙)를 표시한다.
...
- 결과 항목 탭 → 키보드/리스트 닫기 + 해당 박지로 **카메라 이동(줌인)** + 요약 시트 오픈(CS-2 흐름 재사용) + 검색어 유지.
...
- [ ] 마커 탭 → 요약 시트(탭바를 덮고 슬라이드 인, 뒤 지도 조작 가능), 자세히 보기 → 상세, 닫기/스와이프 → 시트 닫힘 + 마커 강조 해제
