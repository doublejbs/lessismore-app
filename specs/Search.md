...
- 모달 옵션: `새 여행`(오늘 날짜로 생성 후 담기) / 기존 배낭 목록(이미 담긴 배낭은 중복 체크로 구분).
...
### SR-10 장비 추가 검색 시트 UI (검색창·이탈 경로) `[제안]`

`/search` 모달(`SearchWarehouseView`, GE-8)의 시트 문법. 2026-08-14 스크린샷 리뷰에서 검색창이
탐색 탭과 다른 Glass(유리 면) 변형으로 남아 세대가 갈리는 것을 확인해 정정한다.

**수용 기준**

- **검색창은 Plain 변형이다** — 탐색 탭(FD-2)과 같은 **연회색 채움 알약**(`Acg.controlFill`,
  radius full, 테두리·유리·그림자 없음, `SearchBarVariant.Plain`).
  - 근거: 이 시트의 지면은 순백(`Acg.paper`)이고, 디자인 시스템 문법(HM-8)대로 **순백 위에서
    흰/유리 면은 보이지 않는다** — Glass 변형을 순백 시트에 두면 면이 사라지고 입력 영역의
    경계가 없어진다.
  - **Glass(유리 면) 변형은 지도·피드 등 콘텐츠 위 오버레이 전용**이다. 순백 지면 화면·시트의
    검색창은 전부 Plain — 화면마다 검색창 문법이 갈리지 않는다. (기존 Feed FD-2의 "장비 추가
    검색 시트는 유리 면 유지" 서술은 이 정정으로 폐기.)
- **우상단 × 닫기 버튼을 두지 않는다.** 이 라우트는 `presentation: 'modal'`(iOS pageSheet,
  `app/_layout.tsx`)이라 OS 그래버가 없어 공용 `SheetGrabberView`를 직접 얹는다(README 시트
  이탈 경로 규칙). ×는 **하단이 주 액션(`확인` 등) 하나뿐인 시트**에만 두는 "적용하지 않고
  나가는 길"인데, 이 시트는 하단 주 액션이 없는 목록형이므로 그래버(+드래그 닫기)만으로
  이탈 경로가 성립한다.
  - 같은 GE-8 얼개의 수동 폼(`/custom`)은 하단 확인 버튼(주 액션)이 있으므로 ×를 **유지**한다
    — 두 화면의 차이는 규칙 적용 결과이지 얼개 이탈이 아니다.
