...
- 검색 결과는 피드 카드와 동일한 **2컬럼 그리드**([Feed.md](Feed.md) FD-2 텍스트 카드 `FeedCardView` 재사용 — 브랜드 → 이름 → 무게 앵커, 우상단 담기 CTA, 장비 이미지 없음)로 표시한다. 마지막 홀수 카드는 좌측 정렬. 제조사 표시는 **`companyKorean || company`** (브랜드 디렉토리·미리보기와 동일 규칙 — 니모/NEMO/nemo 혼재 방지).
