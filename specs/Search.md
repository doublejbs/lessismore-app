...
| `/search`(+`?bagId=`) | 로그인 사용자 | **장비 추가 `검색으로 추가`(GearEdit GE-8) 진입 모달** — 피드 없는 검색 전용(`SearchWarehouseView`). `bagId` 있으면 선택 시 그 배낭에 바로 담기(SR-3) |
...
- 검색 결과는 피드 카드와 동일한 **2컬럼 그리드**([Feed.md](Feed.md) FD-2 카드 `FeedCardView` 재사용 — 정방형 이미지 → 브랜드 → 이름 → 무게, 우상단 담기 CTA)로 표시한다. 마지막 홀수 카드는 좌측 정렬. 제조사 표시는 **`companyKorean || company`** (브랜드 디렉토리·미리보기와 동일 규칙 — 니모/NEMO/nemo 혼재 방지).
...
- **장비 추가 검색 컨텍스트(GearEdit GE-8)** — `+` 클릭 동작이 진입 컨텍스트에 따라 갈린다:
  ...
  - **배낭 장비 추가(`/search?bagId=`)**: 창고 등록 + **그 배낭에 바로 담는다**(배낭 재선택 모달 없음). 이미 그 배낭에 있으면 보유 배지로 표시하고 중복 담기를 막는다. 완료 시 토스트로 알린다.
