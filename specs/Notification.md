...
| 항목 | 내용 |
| --- | --- |
| 상태 | as-built (2026-07-05 구현) · **2026-08-11 개정(as-built)**: 알림 설정 화면을 Liquid Depth로 이식 — 공용 헤더 사용 + 토글 카드 문법(NT-6) · **2026-08-11 개정(as-built)**: 디자인 리뷰 반영 — 토글별 한 줄 설명 + 카드 밖 푸터 안내(NT-6) |
| ID 프리픽스 | `NT` |
...
- **토글마다 한 줄 설명을 붙인다**(2026-08-11 개정 — 이름만으로는 `사용 여부 기록 알림`이 어떤 알림인지 알 수 없었다, 디자인 리뷰). 문구는 실제 예약 규칙에서 옮긴 사실이라 규칙이 바뀌면 함께 고친다: 패킹 `여행 시작 하루 전 저녁 7시에 패킹을 확인하라고 알려요.`(NT-2) · 사용 여부 `여행이 끝난 다음 날 저녁 9시에 쓴 장비를 기록하라고 알려요.`(NT-3) · 공지 `새 소식과 점검 안내를 받아요.`(NT-5).
- **카드 밖에 푸터 안내 두 줄**을 둔다(iOS 설정 관례, 2026-08-11 개정): `알림은 이 기기에만 예약돼요. 앱을 다시 설치하거나 기기를 바꾸면 예약이 사라질 수 있어요.`(NT-4의 알려진 제약) · `알림이 오지 않으면 기기 설정에서 useless 알림 허용을 확인해 주세요.`(NT-1 권한). 설정 값이 아니라 사실이라 카드 안에 넣지 않는다. 화면이 비어 보인다는 지적은 설명·안내로 답하고 채우기용 요소는 두지 않는다 — 설정 화면은 iOS에서도 아래가 비는 것이 정상이다.
...
- **화면 문법 (as-built, 2026-08-11 — Liquid Depth)**: 지형 없는 지면 + 정보 탭 하위 화면 **공용 헤더**(`InfoSubScreenHeaderView` — iOS 네이티브 투명 헤더 / Android·Web 커스텀 헤더 행, [Auth.md](Auth.md) §2)를 쓰고, 토글 세 줄은 **흰 카드 하나** 안에서 좌측 16 들여쓴 헤어라인으로 갈린다. 스위치는 켜짐이 잉크 면이다. 행이 라벨 + 설명 두 줄이 되면서 카드가 커졌으므로 **스크롤 컨테이너로 감싼다**(2026-08-11) — 기본 글자 크기에서는 스크롤이 없고, 큰 글씨(Dynamic Type)에서만 스크롤된다.
