...
- 배낭 `startDate` **전날 저녁(기본 19:00)**에 `{배낭이름} 여행 하루 전! 패킹을 확인해보세요` 로컬 알림을 예약한다.
...
- 배낭 `endDate` **다음날 21:00**에 `{배낭이름} 여행 잘 다녀오셨나요? 사용한 장비를 확인해보세요.` 로컬 알림을 예약한다.
...
- 문구: NT-2 `{배낭이름} 여행 하루 전! 패킹을 확인해보세요`, NT-3 `{배낭이름} 여행 잘 다녀오셨나요? 사용한 장비를 확인해보세요.`
